...
SELECT pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
        pg_catalog.format(
            'starring-runtime-interaction-receipt-acl-backfill-v1:%s:%s',
            :'expected_database',
...
\gset receipt_acl_backfill_

\unset receipt_acl_backfill_lock_acquired

CREATE TEMP TABLE starring_runtime_interaction_receipt_acl_manifest (
...
VALUES
    ('public.guard_runtime_interaction_receipt_event_v1()', FALSE),
    ('public.guard_runtime_interaction_receipt_head_v1()', FALSE),
    ('public.guard_runtime_interaction_receipt_root_v1()', FALSE),
    ('public.guard_runtime_interaction_receipt_token_v1()', FALSE),
    ('public.starring_runtime_interaction_receipt_acknowledgement_finish_v1(text,text,bigint,bigint,text,bytea,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_acknowledgement_intend_v1(text,text,bigint,bigint,text,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_authority_observe_v1(text,text,text,text,text,text,bigint,text,bigint,text,bigint,bigint,bigint,text,text,text,text,text)', TRUE),
    ('public.starring_runtime_interaction_receipt_claim_current_v1(text,text,bigint,text,timestamp with time zone)', FALSE),
    ...
    ('public.starring_runtime_interaction_receipt_execution_intend_v1(text,text,bigint,bigint,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_finish_v1(text,text,bigint,bigint,text,bytea,text,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_plan_bind_v1(text,text,bigint,bigint,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_recover_v1(text,text,bigint,bigint,text,bigint,bigint,bigint,text,text,text,bytea,bigint)', TRUE),
    ('public.starring_runtime_interaction_receipt_scan_recoverable_v1(timestamp with time zone,text,text,timestamp with time zone,text,text,bigint)', TRUE),
    ('public.starring_runtime_interaction_receipt_schema_manifest_v1()', FALSE),
    ('public.starring_runtime_interaction_receipt_terminalize_expired_v1(text,text,bigint,bigint,text,text,bytea)', TRUE),
    ('public.starring_runtime_interaction_receipt_token_expire_v1(text,text,bigint,bigint,bytea)', TRUE);
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill requires PostgreSQL 16'
            USING ERRCODE = '55000';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill database acknowledgement is invalid'
            USING ERRCODE = '55000';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill cluster acknowledgement is invalid'
            USING ERRCODE = '55000';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill requires the fixed cluster administrator'
            USING ERRCODE = '42501';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill requires cluster quiescence'
            USING ERRCODE = '55000';
    END IF;

    IF pg_catalog.to_regclass('public._sqlx_migrations') IS NULL THEN
        RAISE EXCEPTION 'receipt ACL backfill migration ledger is unavailable'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF ledger_count <> 115
        ...
        OR NOT EXISTS (
            SELECT 1
            FROM public._sqlx_migrations AS migration
            WHERE migration.version = 202607310022
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill migration ledger is not exact'
            USING ERRCODE = '55000';
    END IF;

    IF (
        SELECT pg_catalog.count(*)
        FROM pg_temp.starring_runtime_interaction_receipt_acl_manifest
    ) <> 17
        OR (
            SELECT pg_catalog.count(*)
            FROM pg_temp.starring_runtime_interaction_receipt_acl_manifest
        ...
        OR (
            SELECT pg_catalog.count(*)
            FROM pg_temp.starring_runtime_interaction_receipt_role_snapshot
        ) <> 2
    THEN
        RAISE EXCEPTION 'receipt ACL backfill fixed manifest is invalid'
            USING ERRCODE = '55000';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill role attributes are invalid'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill role isolation is invalid'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF pg_catalog.to_regclass('public.automation_instances') IS NULL
        ...
        OR EXISTS (
            SELECT 1
            FROM pg_temp.starring_runtime_interaction_receipt_acl_manifest AS expected
    ...
    THEN
        RAISE EXCEPTION 'receipt ACL backfill ownership or schema is invalid'
            USING ERRCODE = '55000';
    END IF;
END;
$preflight$;
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            THEN
                RAISE EXCEPTION 'receipt ACL backfill grantee is invalid'
                    USING ERRCODE = '55000';
            END IF;
            EXECUTE pg_catalog.format(
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill topology postflight failed'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF EXISTS (
        ...
        (
            SELECT *
            FROM pg_temp.starring_runtime_interaction_receipt_role_setting_snapshot
            EXCEPT ALL
            SELECT setting.setdatabase,
                setting.setrole,
                setting.setconfig
        ...
        RAISE EXCEPTION 'receipt ACL backfill changed role authentication state'
            USING ERRCODE = '55000';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill lost cluster quiescence'
            USING ERRCODE = '55000';
    END IF;
END;
$postflight$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill final proof failed'
            USING ERRCODE = '55000';
    END IF;
END;
$final_role_proof$;
...
\unset receipt_acl_backfill_readiness_proof
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'receipt ACL backfill final quiescence proof failed'
            USING ERRCODE = '55000';
    END IF;
END;
