...
The repeated checkpoint is orchestrated by `matrix.js`. It must run from a clean committed source tree against the exact worker endpoint `http://127.0.0.1:18181`. The dedicated certification worker must report Luna medium, ChatGPT authentication, `codex-cli 0.146.0-alpha.3.1`, concurrency one, queue capacity zero, a 55000 ms request timeout, an idle scheduler, one stable instance ID, and a source digest equal to the local worker modules. The matrix refuses a dirty or changed source tree, a different worker instance or source, a different toolchain identity, and an output path outside the ignored `results/` directory.
