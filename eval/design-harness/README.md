...
The repeated checkpoint is orchestrated by `matrix.js`. It must run from a clean committed source tree against the exact worker endpoint `http://127.0.0.1:18181`. The dedicated certification worker must report Luna medium, ChatGPT authentication, `codex-cli 0.147.0-alpha.6.5`, concurrency one, queue capacity zero, a 55000 ms request timeout, an idle scheduler, one stable instance ID, and a source digest equal to the local worker modules. The matrix refuses a dirty or changed source tree, a different worker instance or source, a different toolchain identity, and an output path outside the ignored `results/` directory.
...
If every gate and all 27 phases completed but a committed evaluator defect prevented final artifacts from being written, `--resume` may perform one deferred finalization from a later clean evaluator commit. This exception is finalization-only: the fixed plan, tooling, original worker instance, worker source, and final request counters must still match; no gate, phase, or model request runs again. The initial failure document and its serialized digest are committed atomically in the state journal before the optional root failure file is written. Deferred finalization restores from that bound journal copy, preserves it as a manifest-bound recovery artifact, and can resume if final artifact writing is interrupted. The manifest keeps the original evidence source and separately records the evaluator commit plus the hashes of `matrix.js`, `acceptance.js`, and `summarize.js`, `mode: deferred_completed_phase_artifacts`, and `model_requests_executed: 0`. A finalized cohort cannot be finalized or overwritten again. Incomplete phase evidence never qualifies for this exception and still requires the original source boundary.

`state.json` is the continuation journal, `phases/` contains the raw phase reports, and `combined.json`, `summary.json`, and `failures.json` are derived evidence. The certification authority is the pair `manifest.json` and `acceptance.json`: the manifest binds the evidence source, any separately attested deferred evaluator, worker, tooling, plan, phases, observed totals, and SHA-256 identities of the derived artifacts; the acceptance artifact carries the model checks and matrix-level certification failures. A pass may be claimed only when both artifacts report `status: "passed"`, `acceptance.json` reports `pass: true`, its certification-failure list is empty, and the manifest-bound artifact digests match. A deferred result additionally requires `state.json` to bind the same completed finalizer and artifacts; root files written before that journal transition are an incomplete transaction. Console output, an uncompleted `state.json`, or `summary.json` alone is not a verdict.
