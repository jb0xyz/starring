...
BEGIN
    ...
    FROM (
        VALUES
            ('public.starring_runtime_writer_fence_observe_v1()', '80fc52622e511a3b1ba21e3391f0246e3dd546f0eef1a841a53ea86aad9d25d8'),
            ('public.starring_runtime_observe_previous_serving_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,text,bigint,text,bigint,text,jsonb)', 'b17ef63bf9d12ee6ac51b2b5d3a77d026f00cb1c5a7b496f00ceb1afc4888afc'),
            ('public.starring_runtime_execution_mutate_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,jsonb)', 'a7305ca9d28ff6416f70485fc3a36790729311d4de2dde5027e7de2f9ee4d438'),
            ('public.starring_runtime_execution_certify_prepare_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint)', '5260a83f8a7f99d98cb59ca3af7eccf7f7596d8a7408cee3e74799dfb10f3d52'),
            ('public.starring_runtime_execution_certify_commit_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint,timestamp with time zone,jsonb,text,jsonb,text)', '150ee8be6a5d4814b26a62e857a7d3dd52ef0d2a7fce4e56c4a3ba61b516ce5f'),
            ('public.starring_runtime_execution_recover_stale_live_v1()', '2d2c7badd87bf0c1dcae2228dd1b8e15c6c57f0dc2805bf0c91e4a0893e0319c'),
            ('public.starring_runtime_serving_heartbeat_v1(text,text,text,text,text,bigint,bigint,bigint,bigint)', '7dc1ccab929dcec92307df363319bbd4600267a9aac6c3c283f4a0f638d1ae0f'),
            ('public.starring_runtime_serving_disconnect_v1(text,text,text,text,text,bigint,bigint,bigint)', '7eeec93e0949056ccf8329587ccee50f5b78279756ad709be94e964d87dba576'),
            ('public.starring_runtime_execution_schema_manifest_v1()', '6cd7af34a993bdfe7a6b86e303d88b4b98bfd5170b34d0a1f15cb81a4aee3a2e'),
            ('public.starring_runtime_execution_database_readiness_v1()', '746a7e3ac38bf588665063c9b3d79df4fc18e278e2a4edf4d1e3ea97e723827d'),
            ('public.starring_runtime_serving_schema_manifest_v1()', 'b61a87b523c89b641e66ce8affbd7904c2a1ee0d29c88131a9a3475becc0c8d2'),
            ('public.starring_runtime_serving_database_readiness_v1()', '5e171ce82811ed7ae17c2e192fc5a8def3f2be0efce162cc3c9faeec484c71d7')
    ...
    WITH identity(group_name, function_identity) AS (
        VALUES
    ...
    ), capability(group_name, function_identity) AS (
        VALUES
            ('execution', 'public.starring_runtime_execution_database_identity_v1()'),
        ...
        LEFT JOIN pg_catalog.pg_proc AS function_row
            ON function_row.oid
                = pg_catalog.to_regprocedure(identity.function_identity)
        LEFT JOIN LATERAL pg_catalog.aclexplode(COALESCE(
            function_row.proacl,
            pg_catalog.acldefault('f', function_row.proowner)
        )) AS privilege ON TRUE
        GROUP BY identity.group_name
    ), observed AS (
        SELECT capability.group_name,
        ...
        LEFT JOIN pg_catalog.pg_proc AS function_row
            ON function_row.oid
                = pg_catalog.to_regprocedure(capability.function_identity)
        LEFT JOIN LATERAL pg_catalog.aclexplode(COALESCE(
            function_row.proacl,
            pg_catalog.acldefault('f', function_row.proowner)
        )) AS privilege ON TRUE
    ...
    FROM observed
    INNER JOIN baseline USING (group_name)
    ...
    IF common_owner IS NULL
        ...
        OR pg_catalog.to_regclass(
            'pg_temp.starring_runtime_lock_order_snapshot'
        ) IS NOT NULL
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_slot_writer_order_preflight_drift';
    END IF;
END;
$preflight$;

CREATE TEMPORARY TABLE pg_temp.starring_runtime_lock_order_snapshot (
...
INSERT INTO pg_temp.starring_runtime_lock_order_snapshot (
    function_identity,
    function_oid,
    function_owner,
    function_acl
...
CREATE OR REPLACE FUNCTION public.starring_runtime_writer_fence_observe_v1()
 RETURNS TABLE(fence_state text, fence_generation bigint, cutover_coordinator_id text, cutover_lease_epoch bigint, database_now timestamp with time zone, cutover_expires_at timestamp with time zone)
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.starring_runtime_observe_previous_serving_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_deployment_revision bigint, expected_controller_id text, expected_controller_fencing_token bigint, expected_convergence_attempt_no bigint, expected_runtime_generation bigint, expected_guild_id text, expected_ruleset_key text, expected_target_version bigint, expected_target_content_hash text, expected_binding_revision bigint, expected_binding_fingerprint text, expected_previous_runtime jsonb)
 RETURNS TABLE(state_name text, observed_at timestamp with time zone, lease_tenant_id text, lease_installation_id text, lease_deployment_id text, lease_attestation_id text, lease_process_instance_id text, lease_runtime_generation bigint, lease_guild_id text, lease_ruleset_key text, lease_target_version bigint, lease_target_content_hash text, lease_binding_revision bigint, lease_binding_fingerprint text, lease_epoch bigint, lease_revision bigint, lease_connected boolean, lease_serving boolean, lease_acquired_at timestamp with time zone, lease_last_heartbeat_at timestamp with time zone, lease_expires_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
DECLARE
...
BEGIN
    ...
    IF NOT FOUND
        OR candidate_guild_id IS DISTINCT FROM expected_guild_id
        OR candidate_ruleset_key IS DISTINCT FROM expected_ruleset_key
    THEN
        RETURN;
    END IF;
...
CREATE OR REPLACE FUNCTION public.starring_runtime_serving_heartbeat_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_attestation_id text, expected_process_instance_id text, expected_runtime_generation bigint, expected_lease_epoch bigint, expected_revision bigint, requested_lease_milliseconds bigint)
 RETURNS TABLE(tenant_id text, installation_id text, deployment_id text, guild_id text, ruleset_key text, attestation_id text, process_instance_id text, runtime_generation bigint, lease_epoch bigint, revision bigint, acquired_at timestamp with time zone, last_heartbeat_at timestamp with time zone, expires_at timestamp with time zone, connected boolean, serving boolean)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RS004',
            MESSAGE = 'runtime_serving_writer_fence_invalid';
    END IF;

    IF writer_fence_state = 'closed' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RS005',
            MESSAGE = 'runtime_serving_writer_fenced';
    END IF;
    ...
    IF NOT FOUND
        OR candidate_guild_id !~ '^[1-9][0-9]{0,19}$'
        OR pg_catalog.length(candidate_guild_id) > 20
        OR (
            pg_catalog.length(candidate_guild_id) = 20
            AND candidate_guild_id COLLATE pg_catalog."C"
                > '18446744073709551615' COLLATE pg_catalog."C"
        )
        OR candidate_ruleset_key !~ '^[A-Za-z0-9_-]{1,64}$'
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RS001',
...
CREATE OR REPLACE FUNCTION public.starring_runtime_serving_disconnect_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_attestation_id text, expected_process_instance_id text, expected_runtime_generation bigint, expected_lease_epoch bigint, expected_revision bigint)
 RETURNS TABLE(tenant_id text, installation_id text, deployment_id text, guild_id text, ruleset_key text, attestation_id text, process_instance_id text, runtime_generation bigint, lease_epoch bigint, revision bigint, acquired_at timestamp with time zone, last_heartbeat_at timestamp with time zone, expires_at timestamp with time zone, connected boolean, serving boolean)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
...
CREATE OR REPLACE FUNCTION public.starring_runtime_execution_certify_prepare_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_deployment_revision bigint, expected_controller_id text, expected_controller_fencing_token bigint, expected_convergence_attempt_no bigint, expected_runtime_generation bigint, expected_gateway_ready jsonb, expected_runtime_build_revision text, expected_panel_report_digest text, expected_gateway_shard_id text, requested_serving_lease_milliseconds bigint)
 RETURNS TABLE(preparation_name text, observed_snapshot jsonb, convergence_attempt_no bigint, mutation_clock timestamp with time zone, certified_at timestamp with time zone)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
...
BEGIN
    ...
    IF NOT FOUND
        OR candidate_guild_id
            IS DISTINCT FROM expected_gateway_ready #>> '{target,guild_id}'
        OR candidate_ruleset_key
            IS DISTINCT FROM expected_gateway_ready #>> '{target,ruleset_key}'
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX001',
        ...
        BEGIN
            ...
            THEN
                ...
                WHERE deployment.tenant_id = candidate_row.tenant_id
                    AND deployment.installation_id = candidate_row.installation_id
                    ...
                    AND deployment.live_attestation_id
                        IS NOT DISTINCT FROM candidate_row.live_attestation_id
                    AND deployment.snapshot IS NOT DISTINCT FROM candidate_row.snapshot
...
CREATE OR REPLACE FUNCTION public.starring_runtime_execution_mutate_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_deployment_revision bigint, expected_controller_id text, expected_controller_fencing_token bigint, expected_convergence_attempt_no bigint, expected_runtime_generation bigint, mutation_kind text, mutation_payload jsonb)
 RETURNS TABLE(outcome_name text, previous_snapshot jsonb, snapshot jsonb, convergence_attempt_no bigint, mutated_at timestamp with time zone)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
...
BEGIN
    ...
    IF NOT FOUND
        OR (
            mutation_kind = 'accept_drain'
            AND (
                deployment_row.guild_id IS DISTINCT FROM candidate_guild_id
                OR deployment_row.ruleset_key
                    IS DISTINCT FROM candidate_ruleset_key
            )
        )
    THEN
        RAISE EXCEPTION USING
...
CREATE OR REPLACE FUNCTION public.starring_runtime_execution_certify_commit_v1(expected_tenant_id text, expected_installation_id text, expected_deployment_id text, expected_deployment_revision bigint, expected_controller_id text, expected_controller_fencing_token bigint, expected_convergence_attempt_no bigint, expected_runtime_generation bigint, expected_gateway_ready jsonb, expected_runtime_build_revision text, expected_panel_report_digest text, expected_gateway_shard_id text, requested_serving_lease_milliseconds bigint, expected_mutation_clock timestamp with time zone, expected_observed_snapshot jsonb, proposed_attestation_id text, proposed_attestation_record jsonb, proposed_attestation_record_bytes text)
 RETURNS TABLE(outcome_name text, previous_snapshot jsonb, snapshot jsonb, convergence_attempt_no bigint, tenant_id text, installation_id text, deployment_id text, guild_id text, ruleset_key text, attestation_id text, process_instance_id text, runtime_generation bigint, lease_epoch bigint, serving_revision bigint, acquired_at timestamp with time zone, last_heartbeat_at timestamp with time zone, expires_at timestamp with time zone, connected boolean, serving boolean)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
...
BEGIN
    ...
    IF NOT FOUND
        OR candidate_guild_id IS DISTINCT FROM
            proposed_attestation_record #>> '{live,target,guild_id}'
        OR candidate_ruleset_key IS DISTINCT FROM
            proposed_attestation_record #>> '{live,target,ruleset_key}'
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX001',
...
CREATE OR REPLACE FUNCTION public.starring_runtime_execution_schema_manifest_v1()
 RETURNS boolean
 LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.starring_runtime_serving_schema_manifest_v1()
 RETURNS boolean
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER
 SET search_path TO 'pg_catalog'
...
CREATE OR REPLACE FUNCTION public.starring_runtime_execution_database_readiness_v1()
 RETURNS TABLE(database_identity text, database_name text, executor_role text, checked_at timestamp with time zone)
 LANGUAGE plpgsql
 STRICT SECURITY DEFINER ROWS 1
 SET search_path TO 'pg_catalog'
AS $function$
...
BEGIN
    ...
    FROM (
        VALUES
            ...
            (
                'public.starring_runtime_gateway_owner_observe_v1(text)',
                'expected_gateway_shard_id text'::TEXT,
                'TABLE(outcome_name text, gateway_shard_id text, process_instance_id text, lease_epoch bigint, expected_build_revision text, owner_revision bigint, database_now timestamp with time zone, expires_at timestamp with time zone)'::TEXT,
                'plpgsql'::TEXT,
                TRUE,
                TRUE,
                1::REAL
            ...
            (
                'public.starring_runtime_gateway_owner_acquire_v1(text,text,text,bigint)',
                'expected_gateway_shard_id text, expected_process_instance_id text, requested_build_revision text, requested_lease_milliseconds bigint'::TEXT,
                'TABLE(outcome_name text, gateway_shard_id text, process_instance_id text, lease_epoch bigint, expected_build_revision text, owner_revision bigint, database_now timestamp with time zone, expires_at timestamp with time zone)'::TEXT,
                'plpgsql'::TEXT,
                TRUE,
                TRUE,
            ...
            (
                'public.starring_runtime_gateway_owner_renew_v1(text,text,bigint,text,bigint,bigint)',
                'expected_gateway_shard_id text, expected_process_instance_id text, expected_lease_epoch bigint, requested_build_revision text, expected_owner_revision bigint, requested_lease_milliseconds bigint'::TEXT,
                'TABLE(outcome_name text, gateway_shard_id text, process_instance_id text, lease_epoch bigint, expected_build_revision text, owner_revision bigint, database_now timestamp with time zone, expires_at timestamp with time zone)'::TEXT,
                'plpgsql'::TEXT,
                TRUE,
                TRUE,
            ...
            (
                'public.starring_runtime_gateway_owner_release_v1(text,text,bigint,text)',
                'expected_gateway_shard_id text, expected_process_instance_id text, expected_lease_epoch bigint, requested_build_revision text'::TEXT,
                'TABLE(outcome_name text, gateway_shard_id text, process_instance_id text, lease_epoch bigint, expected_build_revision text, owner_revision bigint, database_now timestamp with time zone, expires_at timestamp with time zone)'::TEXT,
                'plpgsql'::TEXT,
                TRUE,
                TRUE,
            ...
            (
                'public.starring_runtime_writer_fence_observe_v1()',
                ''::TEXT,
                'TABLE(fence_state text, fence_generation bigint, cutover_coordinator_id text, cutover_lease_epoch bigint, database_now timestamp with time zone, cutover_expires_at timestamp with time zone)'::TEXT,
                'plpgsql'::TEXT,
                TRUE,
                TRUE,
                1::REAL
...
DECLARE
    ...
    invalid_lock_order_count BIGINT;
    writer_observe_source TEXT;
BEGIN
    SELECT relation.relowner
    INTO common_owner
    FROM pg_catalog.pg_class AS relation
    ...
    FROM (
        VALUES
            ('public.starring_runtime_writer_fence_observe_v1()', 'e1d910428439dfa3387988aea6e57d49e3a9a54ba31147f217a19679ed78b5d7'),
            ('public.starring_runtime_observe_previous_serving_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,text,bigint,text,bigint,text,jsonb)', '6d650bd678e95dace59f19f0acb6e03f2a18e44e1707adc0781221c59ba9ced6'),
            ('public.starring_runtime_execution_mutate_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,jsonb)', '7b49bc478d98af25cdf7563a05d3e03ecddbb7fd2ed897a7bcb2f053716fe386'),
            ('public.starring_runtime_execution_certify_prepare_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint)', '84ea51ff6db862974303c191d44e41af00685b3384c732b8dbda1ef7a18df08a'),
            ('public.starring_runtime_execution_certify_commit_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint,timestamp with time zone,jsonb,text,jsonb,text)', 'ddbda720a96466f784b46de120a199d0b20ac3384a39d70c8829d8747532b105'),
            ('public.starring_runtime_execution_recover_stale_live_v1()', '506c532c275fbbe51b1e67d463bf0ddfc71a258e79a6594ab19ea2235c07fc6a'),
            ('public.starring_runtime_serving_heartbeat_v1(text,text,text,text,text,bigint,bigint,bigint,bigint)', '51a3dcd58a44f60320a0bcb5b671fce0eeffa10d08e0a5b8c3880a07ce1802b3'),
            ('public.starring_runtime_serving_disconnect_v1(text,text,text,text,text,bigint,bigint,bigint)', 'f6c6f52bed426d1e95969b457b37782efea34daf566c34aa68714440ef9003b9'),
            ('public.starring_runtime_execution_schema_manifest_v1()', '58520c287c446b96198d624c9e10c4dc7e1ba8371cc721f6192a903685197476'),
            ('public.starring_runtime_execution_database_readiness_v1()', 'c819437ec90f4f64ebd8a3722979e2ea817e87bdc370eef1e5c196e163551188'),
            ('public.starring_runtime_serving_schema_manifest_v1()', '133f73f8eb70606e023af29294ade8bb593b2adc06db3e663bdd42d7693a43be'),
            ('public.starring_runtime_serving_database_readiness_v1()', '45791dded732504e4f235f17153646affa14f6e94e6b4fdc0874f2279e1533a7')
    ) AS expected(identity, definition_digest)
    LEFT JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid = pg_catalog.to_regprocedure(expected.identity)
    LEFT JOIN pg_catalog.pg_language AS language_row
    ...
    FROM pg_temp.starring_runtime_lock_order_snapshot AS snapshot
    LEFT JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid
            = pg_catalog.to_regprocedure(snapshot.function_identity)
    WHERE function_row.oid IS NULL
        OR function_row.oid IS DISTINCT FROM snapshot.function_oid
    ...
    SELECT pg_catalog.count(*)
    INTO invalid_lock_order_count
    FROM (
        VALUES
            ('public.starring_runtime_observe_previous_serving_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,text,bigint,text,bigint,text,jsonb)', 'starring_runtime_writer_fence_observe_v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_execution_mutate_v1(text,text,text,bigint,text,bigint,bigint,bigint,text,jsonb)', 'starring_runtime_writer_fence_observe_v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_execution_certify_prepare_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint)', 'starring_runtime_writer_fence_observe_v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_execution_certify_commit_v1(text,text,text,bigint,text,bigint,bigint,bigint,jsonb,text,text,text,bigint,timestamp with time zone,jsonb,text,jsonb,text)', 'starring_runtime_writer_fence_observe_v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_execution_recover_stale_live_v1()', 'starring_runtime_writer_fence_observe_v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_serving_heartbeat_v1(text,text,text,text,text,bigint,bigint,bigint,bigint)', 'starring-runtime-writer-fence-v1', 'starring-runtime-serving-slot-v1:'),
            ('public.starring_runtime_serving_disconnect_v1(text,text,text,text,text,bigint,bigint,bigint)', 'starring-runtime-writer-fence-v1', 'starring-runtime-serving-slot-v1:')
    ) AS expected(identity, writer_marker, slot_marker)
    LEFT JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid = pg_catalog.to_regprocedure(expected.identity)
    WHERE function_row.oid IS NULL
        OR pg_catalog.strpos(
            function_row.prosrc,
            expected.writer_marker
        ) = 0
        OR pg_catalog.strpos(
            function_row.prosrc,
            expected.slot_marker
        ) = 0
        OR pg_catalog.strpos(function_row.prosrc, 'FOR UPDATE') = 0
        OR pg_catalog.strpos(
            function_row.prosrc,
            expected.writer_marker
        ) >= pg_catalog.strpos(
        ...
        OR pg_catalog.strpos(
            function_row.prosrc,
            expected.slot_marker
        ) >= pg_catalog.strpos(function_row.prosrc, 'FOR UPDATE');

    SELECT function_row.prosrc
    INTO writer_observe_source
    FROM pg_catalog.pg_proc AS function_row
    ...
    IF common_owner IS NULL
        ...
        OR (SELECT pg_catalog.count(*)
            FROM pg_temp.starring_runtime_lock_order_snapshot) <> 12
        OR invalid_lock_order_count <> 0
        OR writer_observe_source IS NULL
        OR pg_catalog.strpos(
            writer_observe_source,
            'pg_advisory_xact_lock_shared'
        ) = 0
        OR pg_catalog.strpos(writer_observe_source, 'FOR SHARE') = 0
        OR pg_catalog.strpos(
            writer_observe_source,
            'pg_advisory_xact_lock_shared'
        ) >= pg_catalog.strpos(writer_observe_source, 'FOR SHARE')
        OR NOT public.starring_runtime_execution_schema_manifest_v1()
        OR NOT public.starring_runtime_serving_schema_manifest_v1()
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_slot_writer_order_postflight_drift';
    END IF;
END;
$postflight$;
