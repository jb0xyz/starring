...
BEGIN
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_exact_target_database_relation_drift';
    END IF;
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_exact_target_database_owner_drift';
    END IF;
    ...
    IF unsafe_schema_create_count <> 0 THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_exact_target_database_schema_authority_drift';
    END IF;
    ...
    WHERE namespace.nspname = 'public'
        AND function_row.proname IN (
            'starring_runtime_exact_target_schema_manifest_v1',
            'starring_runtime_exact_target_database_readiness_v1'
        );
...
BEGIN
    ...
    RETURN observed_count = 354
        AND observed_digest
            = 'cfe8dc4188faae09cb70e99a6f91e6ae3921cfcd0983d85ed42e2649b0cb1d4d';
END;
$function$;
    ...
    LOOP
        ...
        LOOP
            ...
            IF grantee_name IS NULL THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RE001',
                    MESSAGE = 'runtime_exact_target_database_relation_grantee_drift';
            END IF;
            EXECUTE pg_catalog.format(
        ...
        LOOP
            ...
            IF grantee_name IS NULL THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RE001',
                    MESSAGE = 'runtime_exact_target_database_column_grantee_drift';
            END IF;
            EXECUTE pg_catalog.format(
    ...
    LOOP
        ...
        LOOP
            ...
            IF grantee_name IS NULL THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RE001',
                    MESSAGE = 'runtime_exact_target_database_function_grantee_drift';
            END IF;
            EXECUTE pg_catalog.format(
...
BEGIN
    ...
    FROM (
        VALUES
            ...
            (
                ...
                'TABLE(deployment_revision bigint, convergence_attempt_no bigint, installation_authority_revision bigint, current_authority_revision bigint, guild_id text, ruleset_key text, target_version bigint, schema_version bigint, definition jsonb, content_hash text, canonical_content_hash text, created_by text, binding_revision bigint, binding_fingerprint text, resource_bindings jsonb)'::TEXT,
                'sql'::TEXT,
                TRUE,
                TRUE,
                1::REAL
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_exact_target_database_postflight_drift';
    END IF;
END;
$postflight$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_exact_target_database_readiness_postflight_drift';
    END IF;
END;
