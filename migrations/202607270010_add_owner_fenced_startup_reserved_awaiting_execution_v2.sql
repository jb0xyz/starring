SET LOCAL lock_timeout = '5s';
SET LOCAL statement_timeout = '30s';
SET LOCAL search_path = pg_catalog;

SELECT pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('starring-runtime-writer-fence-v1', 0)
);

LOCK TABLE
    public.runtime_writer_fence,
    public.runtime_gateway_owners,
    public.runtime_deployments,
    public.runtime_attestations,
    public.runtime_serving_leases,
    public.runtime_slot_writer_fences_v2,
    public.runtime_certification_operations_v2,
    public.runtime_certification_operation_terminals_v2,
    public.runtime_suspend_attempt_operations_v2,
    public.runtime_suspended_attempts_v2,
    public.runtime_suspend_attempt_completions_v2,
    public.runtime_product_operations_v2,
    public.runtime_drain_intents_v2,
    public.runtime_startup_recovery_actions_v2,
    public.activation_requests,
    public.authoring_promotions,
    public.product_tenants,
    public.automation_installations,
    public.automation_installation_authority_versions,
    public.automation_ruleset_activations,
    public.automation_ruleset_versions
IN ACCESS EXCLUSIVE MODE;

DO $preflight$
DECLARE
    common_owner OID;
    private_schema_owner OID;
    executor_role OID;
    executor_role_is_quarantined BOOLEAN;
    executor_membership_count BIGINT;
    other_client_session_count BIGINT;
    prepared_transaction_count BIGINT;
    collision_count BIGINT;
    manifest_digest TEXT;
    readiness_digest TEXT;
    observation_digest TEXT;
    action_record_digest TEXT;
BEGIN
    SELECT relation.relowner
    INTO common_owner
    FROM pg_catalog.pg_class AS relation
    WHERE relation.oid = pg_catalog.to_regclass(
        'public.runtime_deployments'
    );

    SELECT namespace.nspowner
    INTO private_schema_owner
    FROM pg_catalog.pg_namespace AS namespace
    WHERE namespace.nspname = 'starring_runtime_private_v2';

    SELECT privilege.grantee
    INTO executor_role
    FROM pg_catalog.pg_proc AS capability
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        capability.proacl,
        pg_catalog.acldefault('f', capability.proowner)
    )) AS privilege
    WHERE capability.oid = pg_catalog.to_regprocedure(
            'public.starring_runtime_execution_database_identity_v1()'
        )
        AND privilege.grantee <> common_owner
    ORDER BY privilege.grantee
    LIMIT 1;

    SELECT COALESCE(NOT role.rolcanlogin, TRUE)
    INTO executor_role_is_quarantined
    FROM pg_catalog.pg_roles AS role
    WHERE role.oid = executor_role;
    executor_role_is_quarantined := COALESCE(
        executor_role_is_quarantined,
        executor_role IS NULL
    );

    SELECT pg_catalog.count(*)
    INTO executor_membership_count
    FROM pg_catalog.pg_auth_members AS membership
    WHERE membership.roleid = executor_role
        OR membership.member = executor_role;

    SELECT pg_catalog.count(*)
    INTO other_client_session_count
    FROM pg_catalog.pg_stat_activity AS activity
    WHERE activity.datid = (
            SELECT database_row.oid
            FROM pg_catalog.pg_database AS database_row
            WHERE database_row.datname = pg_catalog.current_database()
        )
        AND activity.pid <> pg_catalog.pg_backend_pid()
        AND activity.backend_type = 'client backend';

    SELECT pg_catalog.count(*)
    INTO prepared_transaction_count
    FROM pg_catalog.pg_prepared_xacts AS prepared
    WHERE prepared.database = pg_catalog.current_database();

    SELECT pg_catalog.count(*)
    INTO collision_count
    FROM pg_catalog.pg_proc AS function_row
    INNER JOIN pg_catalog.pg_namespace AS namespace
        ON namespace.oid = function_row.pronamespace
    WHERE (
            namespace.nspname = 'public'
            AND function_row.proname =
                'starring_runtime_startup_recovery_execute_reserved_awaiting_v2'
        )
        OR (
            namespace.nspname = 'starring_runtime_private_v2'
            AND function_row.proname =
                pg_catalog.left(
                    'starring_runtime_cert_awaiting_reset_exact_v2',
                    63
                )
        )
        OR (
            namespace.nspname = 'starring_runtime_private_v2'
            AND function_row.proname =
                pg_catalog.left(
                    'starring_runtime_startup_reserved_projection_exact_v2',
                    63
                )
        );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO manifest_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_schema_manifest_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO readiness_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_database_readiness_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO observation_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_startup_recovery_observe_v2(text,text,bigint,text,bigint,timestamp with time zone)'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO action_record_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'starring_runtime_private_v2.starring_runtime_startup_recovery_action_record_v2(text,bigint,bigint,bigint,bigint,text,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone,bytea)'
    );

    IF common_owner IS NULL
        OR pg_catalog.to_regrole(current_user) <> common_owner
        OR private_schema_owner IS DISTINCT FROM common_owner
        OR NOT executor_role_is_quarantined
        OR executor_membership_count <> 0
        OR other_client_session_count <> 0
        OR prepared_transaction_count <> 0
        OR collision_count <> 0
        OR EXISTS (
            SELECT 1
            FROM public.runtime_gateway_owners AS owner
            WHERE owner.process_instance_id IS NOT NULL
                AND owner.expires_at > pg_catalog.clock_timestamp()
        )
        OR EXISTS (
            SELECT 1
            FROM public.runtime_serving_leases AS serving
            WHERE serving.connected
                AND serving.serving
                AND serving.expires_at > pg_catalog.clock_timestamp()
        )
        OR manifest_digest IS DISTINCT FROM
            '1fa238c260d3bdfa7b0c914a42616c2889c02d92253829c8bafa63a8a255a3f7'
        OR readiness_digest IS DISTINCT FROM
            'a5191ef59e5365476860af1150a176049ef00c5b0d6c3f7cfe40e0b5be9d738a'
        OR observation_digest IS DISTINCT FROM
            '7153d2dcf3eaa6a6534368eead9f40c157c63372c879ce99adf173eb3d23f306'
        OR action_record_digest IS DISTINCT FROM
            '7f3f6f98d37150b86d3d4ff860018053b402afa27eeaee82694a6dbf4f0e301b'
        OR NOT public.starring_runtime_execution_schema_manifest_v1()
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_preflight_drift';
    END IF;
END;
$preflight$;

CREATE FUNCTION starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(
    previous_deployment public.runtime_deployments,
    proposed_deployment public.runtime_deployments,
    expected_mutation_clock TIMESTAMPTZ
)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
STRICT
PARALLEL UNSAFE
SECURITY INVOKER
SET search_path = pg_catalog
AS $function$
SELECT COALESCE((
    pg_catalog.isfinite(expected_mutation_clock)
    AND previous_deployment.phase IS NOT DISTINCT FROM
        'awaiting_gateway_ready'
    AND previous_deployment.revision
        BETWEEN 1 AND 9223372036854775806
    AND previous_deployment.snapshot -> 'phase'
        IS NOT DISTINCT FROM
        '{"phase":"awaiting_gateway_ready"}'::JSONB
    AND previous_deployment.snapshot -> 'revision'
        IS NOT DISTINCT FROM
        pg_catalog.to_jsonb(previous_deployment.revision)
    AND previous_deployment.controller_id IS NOT NULL
    AND previous_deployment.controller_fencing_token IS NOT NULL
    AND previous_deployment.controller_acquired_at IS NOT NULL
    AND previous_deployment.controller_lease_expires_at IS NOT NULL
    AND previous_deployment.last_controller_id
        IS NOT DISTINCT FROM
        previous_deployment.controller_id
    AND previous_deployment.last_fencing_token
        IS NOT DISTINCT FROM
        previous_deployment.controller_fencing_token
    AND pg_catalog.jsonb_typeof(
        previous_deployment.snapshot -> 'panel_certificate'
    ) IS NOT DISTINCT FROM 'object'
    AND previous_deployment.snapshot -> 'gateway_ready'
        IS NOT DISTINCT FROM 'null'::JSONB
    AND previous_deployment.snapshot -> 'live'
        IS NOT DISTINCT FROM 'null'::JSONB
    AND previous_deployment.live_attestation_id IS NULL
    AND previous_deployment.live_at IS NULL
    AND proposed_deployment.revision IS NOT DISTINCT FROM
        previous_deployment.revision + 1
    AND proposed_deployment.phase IS NOT DISTINCT FROM
        'reconciling_panels'
    AND proposed_deployment.convergence_attempt_no
        IS NOT DISTINCT FROM
        previous_deployment.convergence_attempt_no
    AND proposed_deployment.snapshot -> 'phase'
        IS NOT DISTINCT FROM
        '{"phase":"reconciling_panels"}'::JSONB
    AND proposed_deployment.snapshot -> 'revision'
        IS NOT DISTINCT FROM
        pg_catalog.to_jsonb(proposed_deployment.revision)
    AND proposed_deployment.snapshot -> 'controller_lease'
        IS NOT DISTINCT FROM
        'null'::JSONB
    AND proposed_deployment.snapshot -> 'panel_certificate'
        IS NOT DISTINCT FROM
        'null'::JSONB
    AND proposed_deployment.snapshot -> 'gateway_ready'
        IS NOT DISTINCT FROM
        'null'::JSONB
    AND proposed_deployment.snapshot -> 'live'
        IS NOT DISTINCT FROM 'null'::JSONB
    AND proposed_deployment.controller_id IS NULL
    AND proposed_deployment.controller_fencing_token IS NULL
    AND proposed_deployment.controller_acquired_at IS NULL
    AND proposed_deployment.controller_lease_expires_at IS NULL
    AND proposed_deployment.live_attestation_id IS NULL
    AND proposed_deployment.live_at IS NULL
    AND proposed_deployment.updated_at IS NOT DISTINCT FROM GREATEST(
        expected_mutation_clock,
        previous_deployment.updated_at + INTERVAL '1 microsecond'
    )
    AND pg_catalog.to_jsonb(proposed_deployment) - ARRAY[
        'snapshot',
        'revision',
        'phase',
        'controller_id',
        'controller_fencing_token',
        'controller_acquired_at',
        'controller_lease_expires_at',
        'updated_at'
    ]::TEXT[] IS NOT DISTINCT FROM
        pg_catalog.to_jsonb(previous_deployment) - ARRAY[
            'snapshot',
            'revision',
            'phase',
            'controller_id',
            'controller_fencing_token',
            'controller_acquired_at',
            'controller_lease_expires_at',
            'updated_at'
        ]::TEXT[]
    AND proposed_deployment.snapshot - ARRAY[
        'revision',
        'phase',
        'controller_lease',
        'panel_certificate',
        'gateway_ready',
        'live'
    ]::TEXT[] IS NOT DISTINCT FROM
        previous_deployment.snapshot - ARRAY[
            'revision',
            'phase',
            'controller_lease',
            'panel_certificate',
            'gateway_ready',
            'live'
        ]::TEXT[]
    AND EXISTS (
        SELECT 1
        FROM public.runtime_certification_operations_v2 AS operation
        INNER JOIN public.runtime_certification_operation_terminals_v2
            AS terminal
            ON terminal.operation_id = operation.operation_id
            AND terminal.intent_fingerprint =
                operation.intent_fingerprint
            AND terminal.tenant_id = operation.tenant_id
            AND terminal.installation_id =
                operation.installation_id
            AND terminal.deployment_id = operation.deployment_id
            AND terminal.deployment_revision =
                operation.deployment_revision
            AND terminal.convergence_attempt_no =
                operation.convergence_attempt_no
        WHERE operation.tenant_id = previous_deployment.tenant_id
            AND operation.installation_id =
                previous_deployment.installation_id
            AND operation.deployment_id =
                previous_deployment.deployment_id
            AND operation.deployment_revision =
                previous_deployment.revision
            AND operation.convergence_attempt_no =
                previous_deployment.convergence_attempt_no
            AND terminal.terminal_outcome_name = 'awaiting_reset'
            AND terminal.resulting_phase = 'reconciling_panels'
            AND terminal.resulting_deployment_revision =
                proposed_deployment.revision
            AND terminal.resulting_convergence_attempt_no =
                proposed_deployment.convergence_attempt_no
            AND terminal.terminal_at = expected_mutation_clock
    )
), FALSE);
$function$;

REVOKE ALL ON FUNCTION
    starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(
        public.runtime_deployments,
        public.runtime_deployments,
        TIMESTAMPTZ
    )
FROM PUBLIC;

DO $patch_deployment_validator$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.validate_runtime_deployment_projection()'
    );

    previous_fragment :=
        '    stable_failure_message TEXT;' || E'\n' ||
        'BEGIN';
    next_fragment :=
        '    stable_failure_message TEXT;' || E'\n' ||
        '    certification_awaiting_reset BOOLEAN;' || E'\n' ||
        'BEGIN';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_deployment_validator_declaration_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '    mutation_clock := public.starring_runtime_current_mutation_clock();' || E'\n' ||
        E'\n' ||
        '    snapshot_phase := NEW.snapshot -> ''phase'' ->> ''phase'';';
    next_fragment :=
        '    mutation_clock := public.starring_runtime_current_mutation_clock();' || E'\n' ||
        '    IF TG_OP = ''UPDATE'' THEN' || E'\n' ||
        '        certification_awaiting_reset :=' || E'\n' ||
        '            starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(' || E'\n' ||
        '                OLD,' || E'\n' ||
        '                NEW,' || E'\n' ||
        '                mutation_clock' || E'\n' ||
        '            ) IS TRUE;' || E'\n' ||
        '    ELSE' || E'\n' ||
        '        certification_awaiting_reset := FALSE;' || E'\n' ||
        '    END IF;' || E'\n' ||
        E'\n' ||
        '    snapshot_phase := NEW.snapshot -> ''phase'' ->> ''phase'';';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_deployment_validator_helper_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '            OR (OLD.phase = ''awaiting_gateway_ready'' AND NEW.phase IN (''runtime_pending'',''live'',''superseded''))' || E'\n' ||
        '            OR (OLD.phase = ''live'' AND NEW.phase = ''runtime_pending'')';
    next_fragment :=
        '            OR (OLD.phase = ''awaiting_gateway_ready'' AND NEW.phase IN (''runtime_pending'',''live'',''superseded''))' || E'\n' ||
        '            OR certification_awaiting_reset' || E'\n' ||
        '            OR (OLD.phase = ''live'' AND NEW.phase = ''runtime_pending'')';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_deployment_validator_phase_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_deployment_validator$;

DO $patch_convergence_validator$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.validate_runtime_convergence_attempt_projection()'
    );

    previous_fragment :=
        '    failure_changed BOOLEAN;' || E'\n' ||
        'BEGIN';
    next_fragment :=
        '    failure_changed BOOLEAN;' || E'\n' ||
        '    certification_awaiting_reset BOOLEAN;' || E'\n' ||
        'BEGIN';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_convergence_validator_declaration_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '    mutation_clock := public.starring_runtime_current_mutation_clock();' || E'\n' ||
        '    failure_disposition := NEW.snapshot #>> ''{last_runtime_failure,disposition}'';';
    next_fragment :=
        '    mutation_clock := public.starring_runtime_current_mutation_clock();' || E'\n' ||
        '    IF TG_OP = ''UPDATE'' THEN' || E'\n' ||
        '        certification_awaiting_reset :=' || E'\n' ||
        '            starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(' || E'\n' ||
        '                OLD,' || E'\n' ||
        '                NEW,' || E'\n' ||
        '                mutation_clock' || E'\n' ||
        '            ) IS TRUE;' || E'\n' ||
        '    ELSE' || E'\n' ||
        '        certification_awaiting_reset := FALSE;' || E'\n' ||
        '    END IF;' || E'\n' ||
        '    failure_disposition := NEW.snapshot #>> ''{last_runtime_failure,disposition}'';';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_convergence_validator_helper_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '        IF OLD.controller_id IS NOT NULL' || E'\n' ||
        '            AND NEW.controller_id IS NULL' || E'\n' ||
        '            AND (' || E'\n' ||
        '                OLD.controller_lease_expires_at <= mutation_clock' || E'\n' ||
        '                OR NOT (' || E'\n' ||
        '                    failure_changed' || E'\n' ||
        '                    OR NEW.phase = ''live''' || E'\n' ||
        '                    OR NEW.phase IN (''superseded'', ''cancelled'')' || E'\n' ||
        '                )' || E'\n' ||
        '            )';
    next_fragment :=
        '        IF OLD.controller_id IS NOT NULL' || E'\n' ||
        '            AND NEW.controller_id IS NULL' || E'\n' ||
        '            AND NOT certification_awaiting_reset' || E'\n' ||
        '            AND (' || E'\n' ||
        '                OLD.controller_lease_expires_at <= mutation_clock' || E'\n' ||
        '                OR NOT (' || E'\n' ||
        '                    failure_changed' || E'\n' ||
        '                    OR NEW.phase = ''live''' || E'\n' ||
        '                    OR NEW.phase IN (''superseded'', ''cancelled'')' || E'\n' ||
        '                )' || E'\n' ||
        '            )';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_convergence_validator_release_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_convergence_validator$;

DO $patch_observation$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_startup_recovery_observe_v2(text,text,bigint,text,bigint,timestamp with time zone)'
    );

    previous_fragment :=
        '    PERFORM pg_catalog.pg_advisory_xact_lock(' || E'\n' ||
        '        pg_catalog.hashtextextended(' || E'\n' ||
        '            ''starring-runtime-writer-fence-v1'',' || E'\n' ||
        '            0' || E'\n' ||
        '        )' || E'\n' ||
        '    );';
    next_fragment :=
        '    PERFORM pg_catalog.pg_advisory_xact_lock_shared(' || E'\n' ||
        '        pg_catalog.hashtextextended(' || E'\n' ||
        '            ''starring-runtime-writer-fence-v1'',' || E'\n' ||
        '            0' || E'\n' ||
        '        )' || E'\n' ||
        '    );';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_writer_lock_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '    unresolved_reservation_count BIGINT;' || E'\n' ||
        '    exact_terminal_reservation_count BIGINT;';
    next_fragment :=
        '    unresolved_reservation_count BIGINT;' || E'\n' ||
        '    executable_unresolved_reservation_count BIGINT;' || E'\n' ||
        '    exact_terminal_reservation_count BIGINT;';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_declaration_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '                AND deployment.last_fencing_token =' || E'\n' ||
        '                    deployment.controller_fencing_token' || E'\n' ||
        '        ),' || E'\n' ||
        '        pg_catalog.count(*) FILTER (' || E'\n' ||
        '            WHERE terminal.operation_id IS NOT NULL';
    next_fragment :=
        '                AND deployment.last_fencing_token =' || E'\n' ||
        '                    deployment.controller_fencing_token' || E'\n' ||
        '                AND slot_fence.writer_epoch' || E'\n' ||
        '                    BETWEEN 1 AND 9223372036854775806' || E'\n' ||
        '                AND (' || E'\n' ||
        '                    (' || E'\n' ||
        '                        slot_fence.pending_drain_intent_id IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_product_operation_id IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_tenant_id IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_installation_id IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_deployment_id IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_expected_revision IS NULL' || E'\n' ||
        '                        AND slot_fence.pending_marked_at IS NULL' || E'\n' ||
        '                        AND NOT EXISTS (' || E'\n' ||
        '                            SELECT 1' || E'\n' ||
        '                            FROM public.runtime_drain_intents_v2 AS pending_drain' || E'\n' ||
        '                            WHERE pending_drain.slot_guild_id =' || E'\n' ||
        '                                deployment.guild_id' || E'\n' ||
        '                                AND pending_drain.slot_ruleset_key =' || E'\n' ||
        '                                    deployment.ruleset_key' || E'\n' ||
        '                                AND pending_drain.intent_state = ''pending''' || E'\n' ||
        '                        )' || E'\n' ||
        '                    )' || E'\n' ||
        '                    OR (' || E'\n' ||
        '                        slot_fence.pending_drain_intent_id IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_product_operation_id IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_tenant_id IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_installation_id IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_deployment_id IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_expected_revision IS NOT NULL' || E'\n' ||
        '                        AND slot_fence.pending_marked_at IS NOT NULL' || E'\n' ||
        '                        AND EXISTS (' || E'\n' ||
        '                            SELECT 1' || E'\n' ||
        '                            FROM public.runtime_drain_intents_v2 AS pending_drain' || E'\n' ||
        '                            WHERE pending_drain.drain_intent_id =' || E'\n' ||
        '                                    slot_fence.pending_drain_intent_id' || E'\n' ||
        '                                AND pending_drain.product_operation_id =' || E'\n' ||
        '                                    slot_fence.pending_product_operation_id' || E'\n' ||
        '                                AND pending_drain.tenant_id =' || E'\n' ||
        '                                    slot_fence.pending_tenant_id' || E'\n' ||
        '                                AND pending_drain.installation_id =' || E'\n' ||
        '                                    slot_fence.pending_installation_id' || E'\n' ||
        '                                AND pending_drain.deployment_id =' || E'\n' ||
        '                                    slot_fence.pending_deployment_id' || E'\n' ||
        '                                AND pending_drain.expected_revision =' || E'\n' ||
        '                                    slot_fence.pending_expected_revision' || E'\n' ||
        '                                AND pending_drain.slot_guild_id =' || E'\n' ||
        '                                    deployment.guild_id' || E'\n' ||
        '                                AND pending_drain.slot_ruleset_key =' || E'\n' ||
        '                                    deployment.ruleset_key' || E'\n' ||
        '                                AND pending_drain.intent_state = ''pending''' || E'\n' ||
        '                        )' || E'\n' ||
        '                    )' || E'\n' ||
        '                )' || E'\n' ||
        '        ),' || E'\n' ||
        '        pg_catalog.count(*) FILTER (' || E'\n' ||
        '            WHERE terminal.operation_id IS NULL' || E'\n' ||
        '                AND deployment.phase = ''awaiting_gateway_ready''' || E'\n' ||
        '                AND deployment.revision =' || E'\n' ||
        '                    reservation.deployment_revision' || E'\n' ||
        '                AND deployment.convergence_attempt_no =' || E'\n' ||
        '                    reservation.convergence_attempt_no' || E'\n' ||
        '                AND deployment.snapshot #>> ''{phase,phase}'' =' || E'\n' ||
        '                    ''awaiting_gateway_ready''' || E'\n' ||
        '                AND deployment.snapshot ->> ''revision'' =' || E'\n' ||
        '                    reservation.deployment_revision::TEXT' || E'\n' ||
        '                AND deployment.controller_id IS NOT NULL' || E'\n' ||
        '                AND deployment.controller_fencing_token IS NOT NULL' || E'\n' ||
        '                AND deployment.last_controller_id =' || E'\n' ||
        '                    deployment.controller_id' || E'\n' ||
        '                AND deployment.last_fencing_token =' || E'\n' ||
        '                    deployment.controller_fencing_token' || E'\n' ||
        '                AND slot_fence.writer_epoch' || E'\n' ||
        '                    BETWEEN 1 AND 9223372036854775806' || E'\n' ||
        '                AND slot_fence.pending_drain_intent_id IS NULL' || E'\n' ||
        '                AND slot_fence.pending_product_operation_id IS NULL' || E'\n' ||
        '                AND slot_fence.pending_tenant_id IS NULL' || E'\n' ||
        '                AND slot_fence.pending_installation_id IS NULL' || E'\n' ||
        '                AND slot_fence.pending_deployment_id IS NULL' || E'\n' ||
        '                AND slot_fence.pending_expected_revision IS NULL' || E'\n' ||
        '                AND slot_fence.pending_marked_at IS NULL' || E'\n' ||
        '                AND NOT EXISTS (' || E'\n' ||
        '                    SELECT 1' || E'\n' ||
        '                    FROM public.runtime_drain_intents_v2 AS pending_drain' || E'\n' ||
        '                    WHERE pending_drain.slot_guild_id =' || E'\n' ||
        '                            deployment.guild_id' || E'\n' ||
        '                        AND pending_drain.slot_ruleset_key =' || E'\n' ||
        '                            deployment.ruleset_key' || E'\n' ||
        '                        AND pending_drain.intent_state = ''pending''' || E'\n' ||
        '                )' || E'\n' ||
        '        ),' || E'\n' ||
        '        pg_catalog.count(*) FILTER (' || E'\n' ||
        '            WHERE terminal.operation_id IS NOT NULL';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_unresolved_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '                        AND deployment.convergence_attempt_no >=' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                )' || E'\n' ||
        '        )' || E'\n' ||
        '    INTO' || E'\n' ||
        '        reservation_count,' || E'\n' ||
        '        unresolved_reservation_count,' || E'\n' ||
        '        exact_terminal_reservation_count';
    next_fragment :=
        '                        AND deployment.convergence_attempt_no >=' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                )' || E'\n' ||
        '        )' || E'\n' ||
        '    INTO' || E'\n' ||
        '        reservation_count,' || E'\n' ||
        '        unresolved_reservation_count,' || E'\n' ||
        '        executable_unresolved_reservation_count,' || E'\n' ||
        '        exact_terminal_reservation_count';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_terminal_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '        AND deployment.deployment_id = reservation.deployment_id;' || E'\n' ||
        E'\n' ||
        '    invalid_reservation_count := reservation_count';
    next_fragment :=
        '        AND deployment.deployment_id = reservation.deployment_id' || E'\n' ||
        '    LEFT JOIN public.runtime_slot_writer_fences_v2 AS slot_fence' || E'\n' ||
        '        ON slot_fence.slot_guild_id = deployment.guild_id' || E'\n' ||
        '        AND slot_fence.slot_ruleset_key = deployment.ruleset_key;' || E'\n' ||
        E'\n' ||
        '    invalid_reservation_count := reservation_count';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_slot_join_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '    IF unresolved_reservation_count > 4294967295';
    next_fragment :=
        '    IF executable_unresolved_reservation_count > 4294967295';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_bound_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '    recoverable_awaiting_certification_count :=' || E'\n' ||
        '        unresolved_reservation_count;';
    next_fragment :=
        '    recoverable_awaiting_certification_count :=' || E'\n' ||
        '        executable_unresolved_reservation_count;';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_projection_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '                AND deployment.snapshot #>> ''{phase,phase}'' =' || E'\n' ||
        '                    ''awaiting_gateway_ready''' || E'\n' ||
        '                AND deployment.snapshot ->> ''revision'' =' || E'\n' ||
        '                    reservation.deployment_revision::TEXT';
    next_fragment :=
        '                AND deployment.snapshot -> ''phase'' =' || E'\n' ||
        '                    ''{"phase":"awaiting_gateway_ready"}''::JSONB' || E'\n' ||
        '                AND deployment.snapshot -> ''revision'' =' || E'\n' ||
        '                    pg_catalog.to_jsonb(' || E'\n' ||
        '                        reservation.deployment_revision' || E'\n' ||
        '                    )' || E'\n' ||
        '                AND deployment.revision' || E'\n' ||
        '                    BETWEEN 1 AND 9223372036854775806';
    IF pg_catalog.strpos(definition, previous_fragment) = 0 THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_snapshot_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '            WHERE terminal.operation_id IS NOT NULL' || E'\n' ||
        '                AND deployment.snapshot ->> ''revision'' =' || E'\n' ||
        '                    deployment.revision::TEXT' || E'\n' ||
        '                AND deployment.snapshot #>> ''{phase,phase}'' =' || E'\n' ||
        '                    deployment.phase' || E'\n' ||
        '                AND (' || E'\n' ||
        '                    (' || E'\n' ||
        '                        deployment.revision =' || E'\n' ||
        '                            terminal.resulting_deployment_revision' || E'\n' ||
        '                        AND deployment.phase =' || E'\n' ||
        '                            terminal.resulting_phase' || E'\n' ||
        '                        AND deployment.convergence_attempt_no =' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                    OR (' || E'\n' ||
        '                        deployment.revision >' || E'\n' ||
        '                            terminal.resulting_deployment_revision' || E'\n' ||
        '                        AND deployment.convergence_attempt_no >=' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                )';
    next_fragment :=
        '            WHERE terminal.operation_id IS NOT NULL' || E'\n' ||
        '                AND deployment.snapshot -> ''revision'' =' || E'\n' ||
        '                    pg_catalog.to_jsonb(deployment.revision)' || E'\n' ||
        '                AND (' || E'\n' ||
        '                    (' || E'\n' ||
        '                        deployment.revision =' || E'\n' ||
        '                            terminal.resulting_deployment_revision' || E'\n' ||
        '                        AND deployment.phase =' || E'\n' ||
        '                            terminal.resulting_phase' || E'\n' ||
        '                        AND deployment.snapshot -> ''phase'' =' || E'\n' ||
        '                            pg_catalog.jsonb_build_object(' || E'\n' ||
        '                                ''phase'',' || E'\n' ||
        '                                terminal.resulting_phase' || E'\n' ||
        '                            )' || E'\n' ||
        '                        AND deployment.convergence_attempt_no =' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                    OR (' || E'\n' ||
        '                        deployment.revision >' || E'\n' ||
        '                            terminal.resulting_deployment_revision' || E'\n' ||
        '                        AND deployment.snapshot #>> ''{phase,phase}'' =' || E'\n' ||
        '                            deployment.phase' || E'\n' ||
        '                        AND deployment.convergence_attempt_no >=' || E'\n' ||
        '                            terminal.resulting_convergence_attempt_no' || E'\n' ||
        '                    )' || E'\n' ||
        '                )';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_observation_terminal_snapshot_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_observation$;

CREATE FUNCTION starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(
    projection_bytes BYTEA,
    expected_recovery_id TEXT,
    expected_originating_emergency_generation BIGINT,
    expected_coordinator_generation BIGINT,
    expected_action_authority_revision BIGINT,
    expected_selection_authority_revision BIGINT,
    expected_action_recorded_at TIMESTAMPTZ,
    expected_terminal public.runtime_certification_operation_terminals_v2
)
RETURNS BOOLEAN
LANGUAGE plpgsql
STABLE
STRICT
PARALLEL UNSAFE
SECURITY INVOKER
SET search_path = pg_catalog
AS $function$
DECLARE
    reservation_row public.runtime_certification_operations_v2%ROWTYPE;
    source_deployment public.runtime_deployments%ROWTYPE;
    successor_deployment public.runtime_deployments%ROWTYPE;
    source_slot public.runtime_slot_writer_fences_v2%ROWTYPE;
    successor_slot public.runtime_slot_writer_fences_v2%ROWTYPE;
    projection_prefix BYTEA;
    source_deployment_frame BYTEA;
    successor_deployment_frame BYTEA;
    source_slot_frame BYTEA;
    successor_slot_frame BYTEA;
    reservation_frame BYTEA;
    expected_reservation_frame BYTEA;
    expected_receipt BYTEA;
    expected_terminal_scalar BYTEA;
    frame_bytes BYTEA;
    field_bytes BYTEA;
    domain_bytes BYTEA;
    source_deployment_json JSONB;
    successor_deployment_json JSONB;
    source_slot_json JSONB;
    successor_slot_json JSONB;
    parsed_operation_id TEXT;
    terminal_receipt_digest TEXT;
    projection_length BIGINT;
    cursor_position BIGINT;
    frame_length NUMERIC;
    frame_index INTEGER;
    byte_index INTEGER;
BEGIN
    projection_length := pg_catalog.octet_length(projection_bytes);
    IF projection_length NOT BETWEEN 1 AND 1048576 THEN
        RETURN FALSE;
    END IF;

    domain_bytes := pg_catalog.convert_to(
        'starring.runtime.startup_recovery.reserved_awaiting_certification.terminal.v2',
        'UTF8'
    );
    projection_prefix :=
        pg_catalog.int8send(
            pg_catalog.octet_length(domain_bytes)::BIGINT
        )
        || domain_bytes
        || pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int2send(1::SMALLINT);
    IF pg_catalog.substring(
            projection_bytes,
            1,
            pg_catalog.octet_length(projection_prefix)
        ) IS DISTINCT FROM projection_prefix
        OR projection_length <
            pg_catalog.octet_length(projection_prefix) + 96
    THEN
        RETURN FALSE;
    END IF;

    cursor_position :=
        pg_catalog.octet_length(projection_prefix) + 1;
    parsed_operation_id := pg_catalog.convert_from(
        pg_catalog.substring(
            projection_bytes,
            cursor_position::INTEGER,
            32
        ),
        'UTF8'
    );
    cursor_position := cursor_position + 32;
    terminal_receipt_digest := pg_catalog.convert_from(
        pg_catalog.substring(
            projection_bytes,
            cursor_position::INTEGER,
            64
        ),
        'UTF8'
    );
    cursor_position := cursor_position + 64;
    IF parsed_operation_id !~ '^[0-9a-f]{32}$'
        OR terminal_receipt_digest !~ '^[0-9a-f]{64}$'
        OR expected_terminal.operation_id
            IS DISTINCT FROM parsed_operation_id
        OR expected_terminal.terminal_receipt_digest
            IS DISTINCT FROM terminal_receipt_digest
        OR expected_terminal.terminal_outcome_name
            IS DISTINCT FROM 'awaiting_reset'
        OR expected_terminal.resulting_phase
            IS DISTINCT FROM 'reconciling_panels'
        OR NOT pg_catalog.isfinite(expected_terminal.terminal_at)
        OR expected_terminal.terminal_at > expected_action_recorded_at
    THEN
        RETURN FALSE;
    END IF;

    FOR frame_index IN 1..5 LOOP
        IF cursor_position + 7 > projection_length THEN
            RETURN FALSE;
        END IF;
        frame_length := 0;
        FOR byte_index IN 0..7 LOOP
            frame_length := frame_length * 256
                + pg_catalog.get_byte(
                    projection_bytes,
                    cursor_position::INTEGER - 1 + byte_index
                );
            IF frame_length > 1048576 THEN
                RETURN FALSE;
            END IF;
        END LOOP;
        cursor_position := cursor_position + 8;
        IF frame_length < 1
            OR cursor_position + frame_length - 1 >
                projection_length
        THEN
            RETURN FALSE;
        END IF;
        frame_bytes := pg_catalog.substring(
            projection_bytes,
            cursor_position::INTEGER,
            frame_length::INTEGER
        );
        cursor_position := cursor_position + frame_length::BIGINT;
        CASE frame_index
            WHEN 1 THEN
                source_deployment_frame := frame_bytes;
            WHEN 2 THEN
                successor_deployment_frame := frame_bytes;
            WHEN 3 THEN
                source_slot_frame := frame_bytes;
            WHEN 4 THEN
                successor_slot_frame := frame_bytes;
            WHEN 5 THEN
                reservation_frame := frame_bytes;
        END CASE;
    END LOOP;

    SELECT operation.*
    INTO reservation_row
    FROM public.runtime_certification_operations_v2 AS operation
    WHERE operation.operation_id = parsed_operation_id;
    IF NOT FOUND
        OR reservation_row.intent_fingerprint
            IS DISTINCT FROM expected_terminal.intent_fingerprint
        OR reservation_row.tenant_id
            IS DISTINCT FROM expected_terminal.tenant_id
        OR reservation_row.installation_id
            IS DISTINCT FROM expected_terminal.installation_id
        OR reservation_row.deployment_id
            IS DISTINCT FROM expected_terminal.deployment_id
        OR reservation_row.deployment_revision
            IS DISTINCT FROM expected_terminal.deployment_revision
        OR reservation_row.convergence_attempt_no
            IS DISTINCT FROM expected_terminal.convergence_attempt_no
    THEN
        RETURN FALSE;
    END IF;

    field_bytes := pg_catalog.convert_to(
        reservation_row.operation_id,
        'UTF8'
    );
    expected_reservation_frame :=
        pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.tenant_id,
        'UTF8'
    );
    expected_reservation_frame := expected_reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.installation_id,
        'UTF8'
    );
    expected_reservation_frame := expected_reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.deployment_id,
        'UTF8'
    );
    expected_reservation_frame := expected_reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            reservation_row.deployment_revision
        )
        || pg_catalog.int8send(
            reservation_row.convergence_attempt_no
        )
        || pg_catalog.int8send(
            pg_catalog.octet_length(
                reservation_row.certification_intent_bytes
            )::BIGINT
        )
        || reservation_row.certification_intent_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.intent_fingerprint,
        'UTF8'
    );
    expected_reservation_frame := expected_reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    IF reservation_frame IS DISTINCT FROM
        expected_reservation_frame
    THEN
        RETURN FALSE;
    END IF;

    IF pg_catalog.get_byte(source_deployment_frame, 0) <> 1
        OR pg_catalog.get_byte(successor_deployment_frame, 0) <> 1
        OR pg_catalog.get_byte(source_slot_frame, 0) <> 1
        OR pg_catalog.get_byte(successor_slot_frame, 0) <> 1
    THEN
        RETURN FALSE;
    END IF;
    source_deployment_json := pg_catalog.convert_from(
        pg_catalog.substring(source_deployment_frame, 2),
        'UTF8'
    )::JSONB;
    successor_deployment_json := pg_catalog.convert_from(
        pg_catalog.substring(successor_deployment_frame, 2),
        'UTF8'
    )::JSONB;
    source_slot_json := pg_catalog.convert_from(
        pg_catalog.substring(source_slot_frame, 2),
        'UTF8'
    )::JSONB;
    successor_slot_json := pg_catalog.convert_from(
        pg_catalog.substring(successor_slot_frame, 2),
        'UTF8'
    )::JSONB;
    IF pg_catalog.jsonb_send(source_deployment_json)
            IS DISTINCT FROM source_deployment_frame
        OR pg_catalog.jsonb_send(successor_deployment_json)
            IS DISTINCT FROM successor_deployment_frame
        OR pg_catalog.jsonb_send(source_slot_json)
            IS DISTINCT FROM source_slot_frame
        OR pg_catalog.jsonb_send(successor_slot_json)
            IS DISTINCT FROM successor_slot_frame
    THEN
        RETURN FALSE;
    END IF;
    SELECT populated.*
    INTO STRICT source_deployment
    FROM pg_catalog.jsonb_populate_record(
        NULL::public.runtime_deployments,
        source_deployment_json
    ) AS populated;
    SELECT populated.*
    INTO STRICT successor_deployment
    FROM pg_catalog.jsonb_populate_record(
        NULL::public.runtime_deployments,
        successor_deployment_json
    ) AS populated;
    SELECT populated.*
    INTO STRICT source_slot
    FROM pg_catalog.jsonb_populate_record(
        NULL::public.runtime_slot_writer_fences_v2,
        source_slot_json
    ) AS populated;
    SELECT populated.*
    INTO STRICT successor_slot
    FROM pg_catalog.jsonb_populate_record(
        NULL::public.runtime_slot_writer_fences_v2,
        successor_slot_json
    ) AS populated;

    IF NOT starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(
            source_deployment,
            successor_deployment,
            expected_terminal.terminal_at
        ) IS TRUE
        OR pg_catalog.to_jsonb(source_deployment)
            IS DISTINCT FROM source_deployment_json
        OR pg_catalog.to_jsonb(successor_deployment)
            IS DISTINCT FROM successor_deployment_json
        OR pg_catalog.to_jsonb(source_slot)
            IS DISTINCT FROM source_slot_json
        OR pg_catalog.to_jsonb(successor_slot)
            IS DISTINCT FROM successor_slot_json
        OR source_deployment.tenant_id
            IS DISTINCT FROM reservation_row.tenant_id
        OR source_deployment.installation_id
            IS DISTINCT FROM reservation_row.installation_id
        OR source_deployment.deployment_id
            IS DISTINCT FROM reservation_row.deployment_id
        OR source_deployment.revision
            IS DISTINCT FROM reservation_row.deployment_revision
        OR source_deployment.convergence_attempt_no
            IS DISTINCT FROM reservation_row.convergence_attempt_no
        OR source_slot.slot_guild_id
            IS DISTINCT FROM source_deployment.guild_id
        OR source_slot.slot_ruleset_key
            IS DISTINCT FROM source_deployment.ruleset_key
        OR successor_slot.slot_guild_id
            IS DISTINCT FROM source_slot.slot_guild_id
        OR successor_slot.slot_ruleset_key
            IS DISTINCT FROM source_slot.slot_ruleset_key
        OR source_slot.writer_epoch IS NULL
        OR source_slot.writer_epoch
            NOT BETWEEN 1 AND 9223372036854775806
        OR successor_slot.writer_epoch
            IS DISTINCT FROM source_slot.writer_epoch + 1
        OR pg_catalog.isfinite(source_slot.updated_at) IS NOT TRUE
        OR pg_catalog.isfinite(successor_slot.updated_at) IS NOT TRUE
        OR successor_slot.updated_at < source_slot.updated_at
        OR successor_slot.updated_at < expected_terminal.terminal_at
        OR successor_slot.pending_drain_intent_id IS NOT NULL
        OR successor_slot.pending_product_operation_id IS NOT NULL
        OR successor_slot.pending_tenant_id IS NOT NULL
        OR successor_slot.pending_installation_id IS NOT NULL
        OR successor_slot.pending_deployment_id IS NOT NULL
        OR successor_slot.pending_expected_revision IS NOT NULL
        OR successor_slot.pending_marked_at IS NOT NULL
        OR source_slot.pending_drain_intent_id IS NOT NULL
        OR source_slot.pending_product_operation_id IS NOT NULL
        OR source_slot.pending_tenant_id IS NOT NULL
        OR source_slot.pending_installation_id IS NOT NULL
        OR source_slot.pending_deployment_id IS NOT NULL
        OR source_slot.pending_expected_revision IS NOT NULL
        OR source_slot.pending_marked_at IS NOT NULL
        OR successor_slot_json - ARRAY[
            'writer_epoch',
            'pending_drain_intent_id',
            'pending_product_operation_id',
            'pending_tenant_id',
            'pending_installation_id',
            'pending_deployment_id',
            'pending_expected_revision',
            'pending_marked_at',
            'updated_at'
        ]::TEXT[] IS DISTINCT FROM source_slot_json - ARRAY[
            'writer_epoch',
            'pending_drain_intent_id',
            'pending_product_operation_id',
            'pending_tenant_id',
            'pending_installation_id',
            'pending_deployment_id',
            'pending_expected_revision',
            'pending_marked_at',
            'updated_at'
        ]::TEXT[]
    THEN
        RETURN FALSE;
    END IF;

    domain_bytes := pg_catalog.convert_to(
        'starring.runtime.certification.awaiting_reset.receipt.v2',
        'UTF8'
    );
    field_bytes := pg_catalog.convert_to(
        expected_recovery_id,
        'UTF8'
    );
    expected_receipt :=
        pg_catalog.int8send(
            pg_catalog.octet_length(domain_bytes)::BIGINT
        )
        || domain_bytes
        || pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int2send(1::SMALLINT)
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            expected_originating_emergency_generation
        )
        || pg_catalog.int8send(expected_coordinator_generation)
        || pg_catalog.int8send(
            expected_action_authority_revision
        )
        || pg_catalog.int8send(
            expected_selection_authority_revision
        )
        || pg_catalog.sha256(source_deployment_frame)
        || pg_catalog.sha256(successor_deployment_frame)
        || pg_catalog.sha256(source_slot_frame)
        || pg_catalog.sha256(successor_slot_frame)
        || pg_catalog.sha256(reservation_frame)
        || pg_catalog.timestamptz_send(expected_terminal.terminal_at);
    IF expected_terminal.terminal_receipt_bytes
            IS DISTINCT FROM expected_receipt
        OR expected_terminal.terminal_receipt_digest
            IS DISTINCT FROM
            starring_runtime_private_v2.starring_runtime_certification_terminal_digest_v2(
                2::SMALLINT,
                reservation_row.operation_id,
                reservation_row.intent_fingerprint,
                reservation_row.tenant_id,
                reservation_row.installation_id,
                reservation_row.deployment_id,
                reservation_row.deployment_revision,
                reservation_row.convergence_attempt_no,
                'awaiting_reset',
                'reconciling_panels',
                expected_terminal.resulting_deployment_revision,
                expected_terminal.resulting_convergence_attempt_no,
                expected_terminal.terminal_at,
                expected_receipt
            )
    THEN
        RETURN FALSE;
    END IF;

    expected_terminal_scalar := pg_catalog.int2send(2::SMALLINT);
    field_bytes := pg_catalog.convert_to(
        expected_terminal.terminal_outcome_name,
        'UTF8'
    );
    expected_terminal_scalar := expected_terminal_scalar
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        expected_terminal.resulting_phase,
        'UTF8'
    );
    expected_terminal_scalar := expected_terminal_scalar
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            expected_terminal.resulting_deployment_revision
        )
        || pg_catalog.int8send(
            expected_terminal.resulting_convergence_attempt_no
        )
        || pg_catalog.timestamptz_send(expected_terminal.terminal_at)
        || pg_catalog.int8send(
            pg_catalog.octet_length(
                expected_terminal.terminal_receipt_bytes
            )::BIGINT
        )
        || expected_terminal.terminal_receipt_bytes;
    field_bytes := pg_catalog.convert_to(
        expected_terminal.terminal_receipt_digest,
        'UTF8'
    );
    expected_terminal_scalar := expected_terminal_scalar
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    RETURN pg_catalog.substring(
        projection_bytes,
        cursor_position::INTEGER
    ) IS NOT DISTINCT FROM expected_terminal_scalar;
EXCEPTION
    WHEN OTHERS THEN
        RETURN FALSE;
END;
$function$;


REVOKE ALL ON FUNCTION
    starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(
        BYTEA,
        TEXT,
        BIGINT,
        BIGINT,
        BIGINT,
        BIGINT,
        TIMESTAMPTZ,
        public.runtime_certification_operation_terminals_v2
    )
FROM PUBLIC;

CREATE FUNCTION public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(
    requested_recovery_id TEXT,
    requested_originating_emergency_generation BIGINT,
    requested_coordinator_generation BIGINT,
    requested_action_authority_revision BIGINT,
    requested_selection_authority_revision BIGINT,
    expected_gateway_shard_id TEXT,
    expected_owner_process_instance_id TEXT,
    expected_owner_lease_epoch BIGINT,
    expected_owner_runtime_build_revision TEXT,
    expected_owner_revision BIGINT,
    expected_owner_expires_at TIMESTAMPTZ,
    requested_minimum_database_now TIMESTAMPTZ
)
RETURNS TABLE(
    journal_outcome_name TEXT,
    terminal_outcome_name TEXT,
    recovery_id TEXT,
    originating_emergency_generation BIGINT,
    coordinator_generation BIGINT,
    action_authority_revision BIGINT,
    selection_authority_revision BIGINT,
    recovery_class TEXT,
    observed_gateway_shard_id TEXT,
    observed_process_instance_id TEXT,
    observed_lease_epoch BIGINT,
    observed_runtime_build_revision TEXT,
    observed_owner_revision BIGINT,
    database_now TIMESTAMPTZ,
    observed_owner_expires_at TIMESTAMPTZ,
    minimum_database_now TIMESTAMPTZ,
    recorded_at TIMESTAMPTZ,
    terminal_projection_bytes BYTEA,
    terminal_digest TEXT
)
LANGUAGE plpgsql
VOLATILE
STRICT
PARALLEL UNSAFE
SECURITY DEFINER
SET search_path = pg_catalog
ROWS 1
AS $function$
DECLARE
    owner_row public.runtime_gateway_owners%ROWTYPE;
    selection_action_row public.runtime_startup_recovery_actions_v2%ROWTYPE;
    authority_action_row public.runtime_startup_recovery_actions_v2%ROWTYPE;
    existing_action_row public.runtime_startup_recovery_actions_v2%ROWTYPE;
    reservation_row public.runtime_certification_operations_v2%ROWTYPE;
    terminal_row public.runtime_certification_operation_terminals_v2%ROWTYPE;
    deployment_row public.runtime_deployments%ROWTYPE;
    terminal_deployment_row public.runtime_deployments%ROWTYPE;
    slot_fence_row public.runtime_slot_writer_fences_v2%ROWTYPE;
    terminal_slot_fence_row public.runtime_slot_writer_fences_v2%ROWTYPE;
    action_record RECORD;
    observation_row RECORD;
    selection_action_found BOOLEAN;
    authority_action_found BOOLEAN;
    writer_fence_state TEXT;
    reservation_count BIGINT;
    unresolved_reservation_count BIGINT;
    executable_reservation_count BIGINT;
    exact_terminal_reservation_count BIGINT;
    invalid_reservation_count BIGINT;
    selected_operation_id TEXT;
    replay_operation_id TEXT;
    replay_terminal_digest TEXT;
    slot_writer_epoch BIGINT;
    successor_slot_writer_epoch BIGINT;
    pending_drain_intent_id TEXT;
    pending_product_operation_id TEXT;
    pending_tenant_id TEXT;
    pending_installation_id TEXT;
    pending_deployment_id TEXT;
    pending_expected_revision BIGINT;
    pending_marked_at TIMESTAMPTZ;
    authority_outcome TEXT;
    mutation_clock TIMESTAMPTZ;
    next_revision BIGINT;
    next_snapshot JSONB;
    proposed_deployment JSONB;
    intent_json JSONB;
    expected_intent_bytes BYTEA;
    domain_bytes BYTEA;
    field_bytes BYTEA;
    source_deployment_frame BYTEA;
    successor_deployment_frame BYTEA;
    source_slot_frame BYTEA;
    successor_slot_frame BYTEA;
    reservation_frame BYTEA;
    terminal_receipt_bytes BYTEA;
    certification_terminal_digest TEXT;
    no_candidate_projection BYTEA;
    progressed_projection_prefix BYTEA;
BEGIN
    PERFORM pg_catalog.set_config('TimeZone', 'UTC', TRUE);

    IF pg_catalog.current_setting('transaction_isolation')
            <> 'serializable'
        OR pg_catalog.current_setting('transaction_read_only') <> 'off'
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_transaction_invalid';
    END IF;

    IF requested_selection_authority_revision
            NOT BETWEEN 1 AND 9223372036854775806
        OR requested_recovery_id !~ '^[0-9a-f]{32}$'
        OR requested_originating_emergency_generation
            NOT BETWEEN 1 AND 9223372036854775807
        OR requested_coordinator_generation
            NOT BETWEEN 1 AND 9223372036854775807
        OR requested_action_authority_revision
            NOT BETWEEN 2 AND 9223372036854775807
        OR requested_action_authority_revision
            <> requested_selection_authority_revision + 1
        OR expected_gateway_shard_id IS DISTINCT FROM 'shard:0'
        OR expected_owner_process_instance_id
            !~ '^[A-Za-z0-9_.:-]{1,128}$'
        OR expected_owner_lease_epoch
            NOT BETWEEN 1 AND 9223372036854775807
        OR expected_owner_runtime_build_revision
            !~ '^[A-Za-z0-9_.:/-]{1,128}$'
        OR expected_owner_revision
            NOT BETWEEN 1 AND 9223372036854775807
        OR NOT pg_catalog.isfinite(expected_owner_expires_at)
        OR NOT pg_catalog.isfinite(requested_minimum_database_now)
        OR requested_minimum_database_now >= expected_owner_expires_at
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX002',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_input_invalid';
    END IF;

    domain_bytes := pg_catalog.convert_to(
        'starring.runtime.startup_recovery.reserved_awaiting_certification.terminal.v2',
        'UTF8'
    );
    no_candidate_projection :=
        pg_catalog.int8send(
            pg_catalog.octet_length(domain_bytes)::BIGINT
        )
        || domain_bytes
        || pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int2send(0::SMALLINT);
    progressed_projection_prefix :=
        pg_catalog.int8send(
            pg_catalog.octet_length(domain_bytes)::BIGINT
        )
        || domain_bytes
        || pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int2send(1::SMALLINT);

    SELECT fence.fence_state
    INTO writer_fence_state
    FROM public.starring_runtime_writer_fence_observe_v1() AS fence;
    IF NOT FOUND
        OR writer_fence_state NOT IN ('open', 'closed')
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_writer_fence_invalid';
    END IF;
    IF writer_fence_state = 'closed' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX005',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_writer_fenced';
    END IF;

    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(
            pg_catalog.concat(
                'starring-runtime-gateway-owner-v1:',
                expected_gateway_shard_id
            ),
            0
        )
    );
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(
            pg_catalog.concat(
                'starring-runtime-startup-recovery-action-v2:',
                requested_recovery_id
            ),
            0
        )
    );

    SELECT owner.*
    INTO owner_row
    FROM public.runtime_gateway_owners AS owner
    WHERE owner.gateway_shard_id = expected_gateway_shard_id
    FOR UPDATE;

    database_now := pg_catalog.clock_timestamp();
    IF NOT FOUND
        OR owner_row.process_instance_id
            IS DISTINCT FROM expected_owner_process_instance_id
        OR owner_row.lease_epoch
            IS DISTINCT FROM expected_owner_lease_epoch
        OR owner_row.expected_build_revision
            IS DISTINCT FROM expected_owner_runtime_build_revision
        OR owner_row.owner_revision
            IS DISTINCT FROM expected_owner_revision
        OR owner_row.expires_at
            IS DISTINCT FROM expected_owner_expires_at
        OR owner_row.expires_at <= database_now
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_owner_lost';
    END IF;
    IF database_now < requested_minimum_database_now THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_database_clock_regressed';
    END IF;

    SELECT action.*
    INTO selection_action_row
    FROM public.runtime_startup_recovery_actions_v2 AS action
    WHERE action.recovery_id = requested_recovery_id
        AND action.selection_authority_revision =
            requested_selection_authority_revision
    FOR UPDATE;
    selection_action_found := FOUND;

    SELECT action.*
    INTO authority_action_row
    FROM public.runtime_startup_recovery_actions_v2 AS action
    WHERE action.recovery_id = requested_recovery_id
        AND action.action_authority_revision =
            requested_action_authority_revision
    FOR UPDATE;
    authority_action_found := FOUND;

    IF selection_action_found OR authority_action_found THEN
        IF selection_action_found THEN
            existing_action_row := selection_action_row;
        ELSE
            existing_action_row := authority_action_row;
        END IF;

        SELECT record.*
        INTO STRICT action_record
        FROM starring_runtime_private_v2.starring_runtime_startup_recovery_action_record_v2(
            requested_recovery_id,
            requested_originating_emergency_generation,
            requested_coordinator_generation,
            requested_action_authority_revision,
            requested_selection_authority_revision,
            'reserved_awaiting_certification',
            expected_gateway_shard_id,
            expected_owner_process_instance_id,
            expected_owner_lease_epoch,
            expected_owner_runtime_build_revision,
            expected_owner_revision,
            expected_owner_expires_at,
            requested_minimum_database_now,
            existing_action_row.terminal_projection_bytes
        ) AS record;

        IF action_record.outcome_name IS DISTINCT FROM 'replayed'
            OR action_record.database_now < database_now
            OR action_record.recorded_at > action_record.database_now
            OR action_record.database_now >= expected_owner_expires_at
            OR action_record.observed_gateway_shard_id
                IS DISTINCT FROM expected_gateway_shard_id
            OR action_record.observed_process_instance_id
                IS DISTINCT FROM expected_owner_process_instance_id
            OR action_record.observed_lease_epoch
                IS DISTINCT FROM expected_owner_lease_epoch
            OR action_record.observed_runtime_build_revision
                IS DISTINCT FROM expected_owner_runtime_build_revision
            OR action_record.observed_owner_revision
                IS DISTINCT FROM expected_owner_revision
            OR action_record.observed_owner_expires_at
                IS DISTINCT FROM expected_owner_expires_at
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_replay_invalid';
        END IF;

        IF existing_action_row.terminal_projection_bytes
                IS NOT DISTINCT FROM no_candidate_projection
        THEN
            terminal_outcome_name := 'no_candidate';
        ELSIF pg_catalog.substring(
                existing_action_row.terminal_projection_bytes,
                1,
                pg_catalog.octet_length(progressed_projection_prefix)
            ) IS NOT DISTINCT FROM progressed_projection_prefix
            AND pg_catalog.octet_length(
                existing_action_row.terminal_projection_bytes
            ) > pg_catalog.octet_length(progressed_projection_prefix) + 96
        THEN
            BEGIN
                replay_operation_id := pg_catalog.convert_from(
                    pg_catalog.substring(
                        existing_action_row.terminal_projection_bytes,
                        pg_catalog.octet_length(
                            progressed_projection_prefix
                        ) + 1,
                        32
                    ),
                    'UTF8'
                );
                replay_terminal_digest := pg_catalog.convert_from(
                    pg_catalog.substring(
                        existing_action_row.terminal_projection_bytes,
                        pg_catalog.octet_length(
                            progressed_projection_prefix
                        ) + 33,
                        64
                    ),
                    'UTF8'
                );
            EXCEPTION
                WHEN OTHERS THEN
                    RAISE EXCEPTION USING
                        ERRCODE = 'RX004',
                        MESSAGE = 'runtime_startup_reserved_awaiting_execution_projection_invalid';
            END;
            IF replay_operation_id !~ '^[0-9a-f]{32}$'
                OR replay_terminal_digest !~ '^[0-9a-f]{64}$'
            THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RX004',
                    MESSAGE = 'runtime_startup_reserved_awaiting_execution_projection_invalid';
            END IF;

            SELECT terminal.*
            INTO terminal_row
            FROM public.runtime_certification_operation_terminals_v2
                AS terminal
            WHERE terminal.operation_id = replay_operation_id
                AND terminal.terminal_receipt_digest =
                    replay_terminal_digest
                AND terminal.terminal_outcome_name = 'awaiting_reset'
                AND terminal.resulting_phase = 'reconciling_panels'
            FOR SHARE;
            IF NOT FOUND
                OR NOT starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(
                    existing_action_row.terminal_projection_bytes,
                    requested_recovery_id,
                    requested_originating_emergency_generation,
                    requested_coordinator_generation,
                    requested_action_authority_revision,
                    requested_selection_authority_revision,
                    action_record.recorded_at,
                    terminal_row
                ) IS TRUE
            THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RX004',
                    MESSAGE = 'runtime_startup_reserved_awaiting_execution_replay_terminal_invalid';
            END IF;

            SELECT deployment.*
            INTO terminal_deployment_row
            FROM public.runtime_deployments AS deployment
            WHERE deployment.tenant_id = terminal_row.tenant_id
                AND deployment.installation_id =
                    terminal_row.installation_id
                AND deployment.deployment_id =
                    terminal_row.deployment_id
            FOR SHARE;
            IF NOT FOUND
                OR terminal_deployment_row.snapshot -> 'revision'
                    IS DISTINCT FROM
                    pg_catalog.to_jsonb(
                        terminal_deployment_row.revision
                    )
                OR NOT (
                    (
                        terminal_deployment_row.revision =
                            terminal_row.resulting_deployment_revision
                        AND terminal_deployment_row.phase =
                            terminal_row.resulting_phase
                        AND terminal_deployment_row.snapshot -> 'phase'
                            IS NOT DISTINCT FROM
                            pg_catalog.jsonb_build_object(
                                'phase',
                                terminal_row.resulting_phase
                            )
                        AND terminal_deployment_row.convergence_attempt_no =
                            terminal_row.resulting_convergence_attempt_no
                    )
                    OR (
                        terminal_deployment_row.revision >
                            terminal_row.resulting_deployment_revision
                        AND terminal_deployment_row.snapshot
                            #>> '{phase,phase}'
                            IS NOT DISTINCT FROM
                            terminal_deployment_row.phase
                        AND terminal_deployment_row.convergence_attempt_no >=
                            terminal_row.resulting_convergence_attempt_no
                    )
                )
            THEN
                RAISE EXCEPTION USING
                    ERRCODE = 'RX004',
                    MESSAGE = 'runtime_startup_reserved_awaiting_execution_replay_state_invalid';
            END IF;
            terminal_outcome_name := 'progressed';
        ELSE
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_projection_invalid';
        END IF;

        journal_outcome_name := action_record.outcome_name;
        recovery_id := requested_recovery_id;
        originating_emergency_generation :=
            requested_originating_emergency_generation;
        coordinator_generation := requested_coordinator_generation;
        action_authority_revision :=
            requested_action_authority_revision;
        selection_authority_revision :=
            requested_selection_authority_revision;
        recovery_class := 'reserved_awaiting_certification';
        observed_gateway_shard_id :=
            action_record.observed_gateway_shard_id;
        observed_process_instance_id :=
            action_record.observed_process_instance_id;
        observed_lease_epoch := action_record.observed_lease_epoch;
        observed_runtime_build_revision :=
            action_record.observed_runtime_build_revision;
        observed_owner_revision :=
            action_record.observed_owner_revision;
        database_now := action_record.database_now;
        observed_owner_expires_at :=
            action_record.observed_owner_expires_at;
        minimum_database_now := requested_minimum_database_now;
        recorded_at := action_record.recorded_at;
        terminal_projection_bytes :=
            existing_action_row.terminal_projection_bytes;
        terminal_digest := action_record.terminal_digest;
        RETURN NEXT;
        RETURN;
    END IF;

    WITH inventory AS (
        SELECT
            reservation.operation_id,
            deployment.requested_at,
            deployment.deployment_id,
            reservation.deployment_revision,
            reservation.convergence_attempt_no,
            (
                deployment.phase = 'awaiting_gateway_ready'
                AND deployment.revision =
                    reservation.deployment_revision
                AND deployment.revision
                    BETWEEN 1 AND 9223372036854775806
                AND deployment.convergence_attempt_no =
                    reservation.convergence_attempt_no
                AND deployment.snapshot -> 'phase' =
                    '{"phase":"awaiting_gateway_ready"}'::JSONB
                AND deployment.snapshot -> 'revision' =
                    pg_catalog.to_jsonb(
                        reservation.deployment_revision
                    )
                AND deployment.controller_id IS NOT NULL
                AND deployment.controller_fencing_token IS NOT NULL
                AND deployment.last_controller_id =
                    deployment.controller_id
                AND deployment.last_fencing_token =
                    deployment.controller_fencing_token
            ) AS deployment_is_exact,
            (
                slot_fence.writer_epoch
                    BETWEEN 1 AND 9223372036854775806
                AND slot_fence.pending_drain_intent_id IS NULL
                AND slot_fence.pending_product_operation_id IS NULL
                AND slot_fence.pending_tenant_id IS NULL
                AND slot_fence.pending_installation_id IS NULL
                AND slot_fence.pending_deployment_id IS NULL
                AND slot_fence.pending_expected_revision IS NULL
                AND slot_fence.pending_marked_at IS NULL
                AND NOT EXISTS (
                    SELECT 1
                    FROM public.runtime_drain_intents_v2 AS pending_drain
                    WHERE pending_drain.slot_guild_id =
                            deployment.guild_id
                        AND pending_drain.slot_ruleset_key =
                            deployment.ruleset_key
                        AND pending_drain.intent_state = 'pending'
                )
            ) AS slot_is_clear,
            (
                slot_fence.writer_epoch
                    BETWEEN 1 AND 9223372036854775806
                AND slot_fence.pending_drain_intent_id IS NOT NULL
                AND slot_fence.pending_product_operation_id IS NOT NULL
                AND slot_fence.pending_tenant_id IS NOT NULL
                AND slot_fence.pending_installation_id IS NOT NULL
                AND slot_fence.pending_deployment_id IS NOT NULL
                AND slot_fence.pending_expected_revision IS NOT NULL
                AND slot_fence.pending_marked_at IS NOT NULL
                AND EXISTS (
                    SELECT 1
                    FROM public.runtime_drain_intents_v2 AS pending_drain
                    WHERE pending_drain.drain_intent_id =
                            slot_fence.pending_drain_intent_id
                        AND pending_drain.product_operation_id =
                            slot_fence.pending_product_operation_id
                        AND pending_drain.tenant_id =
                            slot_fence.pending_tenant_id
                        AND pending_drain.installation_id =
                            slot_fence.pending_installation_id
                        AND pending_drain.deployment_id =
                            slot_fence.pending_deployment_id
                        AND pending_drain.expected_revision =
                            slot_fence.pending_expected_revision
                        AND pending_drain.slot_guild_id =
                            deployment.guild_id
                        AND pending_drain.slot_ruleset_key =
                            deployment.ruleset_key
                        AND pending_drain.intent_state = 'pending'
                )
            ) AS slot_is_blocked,
            terminal.operation_id IS NULL AS terminal_is_absent,
            (
                terminal.operation_id IS NOT NULL
                AND deployment.snapshot -> 'revision' =
                    pg_catalog.to_jsonb(deployment.revision)
                AND (
                    (
                        deployment.revision =
                            terminal.resulting_deployment_revision
                        AND deployment.phase =
                            terminal.resulting_phase
                        AND deployment.snapshot -> 'phase' =
                            pg_catalog.jsonb_build_object(
                                'phase',
                                terminal.resulting_phase
                            )
                        AND deployment.convergence_attempt_no =
                            terminal.resulting_convergence_attempt_no
                    )
                    OR (
                        deployment.revision >
                            terminal.resulting_deployment_revision
                        AND deployment.snapshot #>> '{phase,phase}' =
                            deployment.phase
                        AND deployment.convergence_attempt_no >=
                            terminal.resulting_convergence_attempt_no
                    )
                )
            ) AS terminal_history_is_exact
        FROM public.runtime_certification_operations_v2 AS reservation
        LEFT JOIN public.runtime_certification_operation_terminals_v2
            AS terminal
            ON terminal.operation_id = reservation.operation_id
            AND terminal.intent_fingerprint =
                reservation.intent_fingerprint
            AND terminal.tenant_id = reservation.tenant_id
            AND terminal.installation_id =
                reservation.installation_id
            AND terminal.deployment_id = reservation.deployment_id
            AND terminal.deployment_revision =
                reservation.deployment_revision
            AND terminal.convergence_attempt_no =
                reservation.convergence_attempt_no
        LEFT JOIN public.runtime_deployments AS deployment
            ON deployment.tenant_id = reservation.tenant_id
            AND deployment.installation_id =
                reservation.installation_id
            AND deployment.deployment_id = reservation.deployment_id
        LEFT JOIN public.runtime_slot_writer_fences_v2 AS slot_fence
            ON slot_fence.slot_guild_id = deployment.guild_id
            AND slot_fence.slot_ruleset_key = deployment.ruleset_key
    ), categorized AS (
        SELECT
            inventory.*,
            (
                COALESCE(inventory.deployment_is_exact, FALSE)
                AND COALESCE(inventory.slot_is_clear, FALSE)
                AND inventory.terminal_is_absent
            ) AS is_executable,
            (
                COALESCE(inventory.deployment_is_exact, FALSE)
                AND (
                    COALESCE(inventory.slot_is_clear, FALSE)
                    OR COALESCE(inventory.slot_is_blocked, FALSE)
                )
                AND inventory.terminal_is_absent
            ) AS is_unresolved,
            COALESCE(
                inventory.terminal_history_is_exact,
                FALSE
            ) AS is_exact_terminal
        FROM inventory
    ), counts AS (
        SELECT
            pg_catalog.count(*) AS reservation_count,
            pg_catalog.count(*) FILTER (
                WHERE categorized.is_unresolved
            ) AS unresolved_reservation_count,
            pg_catalog.count(*) FILTER (
                WHERE categorized.is_executable
            ) AS executable_reservation_count,
            pg_catalog.count(*) FILTER (
                WHERE categorized.is_exact_terminal
            ) AS exact_terminal_reservation_count
        FROM categorized
    )
    SELECT
        counts.reservation_count,
        counts.unresolved_reservation_count,
        counts.executable_reservation_count,
        counts.exact_terminal_reservation_count,
        (
            SELECT categorized.operation_id
            FROM categorized
            WHERE categorized.is_executable
            ORDER BY
                categorized.requested_at,
                categorized.deployment_id COLLATE pg_catalog."C",
                categorized.deployment_revision,
                categorized.convergence_attempt_no,
                categorized.operation_id COLLATE pg_catalog."C"
            LIMIT 1
        )
    INTO
        reservation_count,
        unresolved_reservation_count,
        executable_reservation_count,
        exact_terminal_reservation_count,
        selected_operation_id
    FROM counts;

    invalid_reservation_count := reservation_count
        - unresolved_reservation_count
        - exact_terminal_reservation_count;
    IF invalid_reservation_count <> 0
        OR executable_reservation_count > 4294967295
        OR (
            executable_reservation_count = 0
            AND selected_operation_id IS NOT NULL
        )
        OR (
            executable_reservation_count <> 0
            AND selected_operation_id IS NULL
        )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_state_ambiguous';
    END IF;

    IF executable_reservation_count = 0
        AND unresolved_reservation_count <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX007',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_product_drain_pending';
    END IF;

    IF executable_reservation_count = 0 THEN
        SELECT observation.*
        INTO STRICT observation_row
        FROM public.starring_runtime_startup_recovery_observe_v2(
            expected_gateway_shard_id,
            expected_owner_process_instance_id,
            expected_owner_lease_epoch,
            expected_owner_runtime_build_revision,
            expected_owner_revision,
            expected_owner_expires_at
        ) AS observation;
        IF observation_row.outcome_name = 'not_current' THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX001',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_owner_lost';
        ELSIF observation_row.outcome_name IS DISTINCT FROM 'observed'
            OR observation_row.serving_state_name = 'ambiguous'
            OR observation_row.recoverable_awaiting_certification_count
                IS DISTINCT FROM 0
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_state_ambiguous';
        ELSIF observation_row.serving_state_name =
            'recoverable_stale'
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX003',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_higher_priority_pending';
        END IF;

        SELECT record.*
        INTO STRICT action_record
        FROM starring_runtime_private_v2.starring_runtime_startup_recovery_action_record_v2(
            requested_recovery_id,
            requested_originating_emergency_generation,
            requested_coordinator_generation,
            requested_action_authority_revision,
            requested_selection_authority_revision,
            'reserved_awaiting_certification',
            expected_gateway_shard_id,
            expected_owner_process_instance_id,
            expected_owner_lease_epoch,
            expected_owner_runtime_build_revision,
            expected_owner_revision,
            expected_owner_expires_at,
            requested_minimum_database_now,
            no_candidate_projection
        ) AS record;
        IF action_record.outcome_name IS DISTINCT FROM 'applied'
            OR action_record.database_now < database_now
            OR action_record.recorded_at < database_now
            OR action_record.database_now
                IS DISTINCT FROM action_record.recorded_at
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_record_invalid';
        END IF;

        journal_outcome_name := action_record.outcome_name;
        terminal_outcome_name := 'no_candidate';
        recovery_id := requested_recovery_id;
        originating_emergency_generation :=
            requested_originating_emergency_generation;
        coordinator_generation := requested_coordinator_generation;
        action_authority_revision :=
            requested_action_authority_revision;
        selection_authority_revision :=
            requested_selection_authority_revision;
        recovery_class := 'reserved_awaiting_certification';
        observed_gateway_shard_id :=
            action_record.observed_gateway_shard_id;
        observed_process_instance_id :=
            action_record.observed_process_instance_id;
        observed_lease_epoch := action_record.observed_lease_epoch;
        observed_runtime_build_revision :=
            action_record.observed_runtime_build_revision;
        observed_owner_revision :=
            action_record.observed_owner_revision;
        database_now := action_record.database_now;
        observed_owner_expires_at :=
            action_record.observed_owner_expires_at;
        minimum_database_now := requested_minimum_database_now;
        recorded_at := action_record.recorded_at;
        terminal_projection_bytes := no_candidate_projection;
        terminal_digest := action_record.terminal_digest;
        RETURN NEXT;
        RETURN;
    END IF;

    SELECT deployment.*
    INTO deployment_row
    FROM public.runtime_certification_operations_v2 AS reservation
    INNER JOIN public.runtime_deployments AS deployment
        ON deployment.tenant_id = reservation.tenant_id
        AND deployment.installation_id =
            reservation.installation_id
        AND deployment.deployment_id = reservation.deployment_id
    WHERE reservation.operation_id = selected_operation_id;
    IF NOT FOUND
        OR deployment_row.guild_id !~ '^[1-9][0-9]{0,19}$'
        OR (
            pg_catalog.length(deployment_row.guild_id) = 20
            AND deployment_row.guild_id COLLATE pg_catalog."C"
                > '18446744073709551615' COLLATE pg_catalog."C"
        )
        OR deployment_row.ruleset_key
            !~ '^[A-Za-z0-9_-]{1,64}$'
    THEN
        RAISE EXCEPTION USING
            ERRCODE = '40001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_selection_changed';
    END IF;

    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(
            pg_catalog.concat(
                'starring-runtime-serving-slot-v1:',
                deployment_row.guild_id,
                ':',
                deployment_row.ruleset_key
            ),
            0
        )
    );

    SELECT
        fence.writer_epoch,
        fence.pending_drain_intent_id,
        fence.pending_product_operation_id,
        fence.pending_tenant_id,
        fence.pending_installation_id,
        fence.pending_deployment_id,
        fence.pending_expected_revision,
        fence.pending_marked_at
    INTO
        slot_writer_epoch,
        pending_drain_intent_id,
        pending_product_operation_id,
        pending_tenant_id,
        pending_installation_id,
        pending_deployment_id,
        pending_expected_revision,
        pending_marked_at
    FROM starring_runtime_private_v2.starring_runtime_slot_writer_fence_lock_v2(
        deployment_row.guild_id,
        deployment_row.ruleset_key
    ) AS fence;
    IF NOT FOUND
        OR slot_writer_epoch NOT BETWEEN 1 AND 9223372036854775806
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_slot_invalid';
    END IF;
    IF pending_drain_intent_id IS NOT NULL THEN
        IF pending_product_operation_id IS NULL
            OR pending_tenant_id IS NULL
            OR pending_installation_id IS NULL
            OR pending_deployment_id IS NULL
            OR pending_expected_revision IS NULL
            OR pending_marked_at IS NULL
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_drain_state_invalid';
        END IF;
        RAISE EXCEPTION USING
            ERRCODE = 'RX007',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_product_drain_pending';
    ELSIF pending_product_operation_id IS NOT NULL
        OR pending_tenant_id IS NOT NULL
        OR pending_installation_id IS NOT NULL
        OR pending_deployment_id IS NOT NULL
        OR pending_expected_revision IS NOT NULL
        OR pending_marked_at IS NOT NULL
        OR EXISTS (
            SELECT 1
            FROM public.runtime_drain_intents_v2 AS pending_drain
            WHERE pending_drain.slot_guild_id =
                    deployment_row.guild_id
                AND pending_drain.slot_ruleset_key =
                    deployment_row.ruleset_key
                AND pending_drain.intent_state = 'pending'
        )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_drain_state_invalid';
    END IF;

    SELECT fence.*
    INTO slot_fence_row
    FROM public.runtime_slot_writer_fences_v2 AS fence
    WHERE fence.slot_guild_id = deployment_row.guild_id
        AND fence.slot_ruleset_key = deployment_row.ruleset_key
        AND fence.writer_epoch = slot_writer_epoch
    FOR UPDATE;
    IF NOT FOUND
        OR slot_fence_row.pending_drain_intent_id IS NOT NULL
        OR slot_fence_row.pending_product_operation_id IS NOT NULL
        OR slot_fence_row.pending_tenant_id IS NOT NULL
        OR slot_fence_row.pending_installation_id IS NOT NULL
        OR slot_fence_row.pending_deployment_id IS NOT NULL
        OR slot_fence_row.pending_expected_revision IS NOT NULL
        OR slot_fence_row.pending_marked_at IS NOT NULL
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_slot_invalid';
    END IF;

    SELECT deployment.*
    INTO deployment_row
    FROM public.runtime_deployments AS deployment
    WHERE deployment.tenant_id = deployment_row.tenant_id
        AND deployment.installation_id =
            deployment_row.installation_id
        AND deployment.deployment_id = deployment_row.deployment_id
    FOR UPDATE;
    IF NOT FOUND
        OR deployment_row.phase <> 'awaiting_gateway_ready'
        OR deployment_row.snapshot -> 'phase'
            IS DISTINCT FROM
            '{"phase":"awaiting_gateway_ready"}'::JSONB
        OR deployment_row.snapshot -> 'revision'
            IS DISTINCT FROM
            pg_catalog.to_jsonb(deployment_row.revision)
        OR deployment_row.controller_id IS NULL
        OR deployment_row.controller_fencing_token IS NULL
        OR deployment_row.controller_acquired_at IS NULL
        OR deployment_row.controller_lease_expires_at IS NULL
        OR deployment_row.last_controller_id
            IS DISTINCT FROM deployment_row.controller_id
        OR deployment_row.last_fencing_token
            IS DISTINCT FROM deployment_row.controller_fencing_token
        OR pg_catalog.jsonb_typeof(
            deployment_row.snapshot -> 'panel_certificate'
        ) <> 'object'
        OR deployment_row.snapshot -> 'gateway_ready'
            IS DISTINCT FROM 'null'::JSONB
        OR deployment_row.snapshot -> 'live'
            IS DISTINCT FROM 'null'::JSONB
        OR deployment_row.live_attestation_id IS NOT NULL
        OR deployment_row.live_at IS NOT NULL
        OR deployment_row.revision NOT BETWEEN 1 AND 9223372036854775806
    THEN
        RAISE EXCEPTION USING
            ERRCODE = '40001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_selection_changed';
    END IF;

    SELECT reservation.*
    INTO reservation_row
    FROM public.runtime_certification_operations_v2 AS reservation
    WHERE reservation.operation_id = selected_operation_id
        AND reservation.tenant_id = deployment_row.tenant_id
        AND reservation.installation_id =
            deployment_row.installation_id
        AND reservation.deployment_id = deployment_row.deployment_id
        AND reservation.deployment_revision = deployment_row.revision
        AND reservation.convergence_attempt_no =
            deployment_row.convergence_attempt_no
    FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION USING
            ERRCODE = '40001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_selection_changed';
    END IF;

    SELECT terminal.*
    INTO terminal_row
    FROM public.runtime_certification_operation_terminals_v2
        AS terminal
    WHERE terminal.operation_id = reservation_row.operation_id
    FOR UPDATE;
    IF FOUND THEN
        RAISE EXCEPTION USING
            ERRCODE = '40001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_selection_changed';
    END IF;

    SELECT observation.*
    INTO STRICT observation_row
    FROM public.starring_runtime_startup_recovery_observe_v2(
        expected_gateway_shard_id,
        expected_owner_process_instance_id,
        expected_owner_lease_epoch,
        expected_owner_runtime_build_revision,
        expected_owner_revision,
        expected_owner_expires_at
    ) AS observation;
    IF observation_row.outcome_name = 'not_current' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_owner_lost';
    ELSIF observation_row.outcome_name IS DISTINCT FROM 'observed'
        OR observation_row.serving_state_name = 'ambiguous'
        OR observation_row.recoverable_awaiting_certification_count
            IS DISTINCT FROM executable_reservation_count
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_state_ambiguous';
    ELSIF observation_row.serving_state_name = 'recoverable_stale' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX003',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_higher_priority_pending';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM public.runtime_attestations AS attestation
        WHERE attestation.tenant_id = deployment_row.tenant_id
            AND attestation.installation_id =
                deployment_row.installation_id
            AND attestation.deployment_id =
                deployment_row.deployment_id
            AND attestation.deployment_revision =
                deployment_row.revision + 1
        FOR SHARE
    ) OR EXISTS (
        SELECT 1
        FROM public.runtime_serving_leases AS serving
        WHERE serving.tenant_id = deployment_row.tenant_id
            AND serving.installation_id =
                deployment_row.installation_id
            AND serving.deployment_id = deployment_row.deployment_id
        FOR SHARE
    )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_attestation_invalid';
    END IF;

    BEGIN
        intent_json := pg_catalog.convert_from(
            reservation_row.certification_intent_bytes,
            'UTF8'
        )::JSONB;
        IF pg_catalog.jsonb_typeof(intent_json) <> 'object'
            OR NOT pg_catalog.pg_input_is_valid(
                intent_json ->> 'action_id',
                'bigint'
            )
            OR NOT pg_catalog.pg_input_is_valid(
                intent_json #>> '{gateway_owner_lease_id,lease_epoch}',
                'bigint'
            )
            OR NOT pg_catalog.pg_input_is_valid(
                intent_json ->> 'observed_owner_revision',
                'bigint'
            )
            OR NOT pg_catalog.pg_input_is_valid(
                intent_json ->> 'serving_lease_milliseconds',
                'bigint'
            )
            OR intent_json #>> '{gateway_owner_lease_id,gateway_shard_id}'
                IS DISTINCT FROM 'shard:0'
            OR intent_json #>> '{gateway_owner_lease_id,process_instance_id}'
                IS DISTINCT FROM deployment_row.snapshot
                    #>> '{panel_certificate,process_instance_id}'
            OR intent_json #>> '{process_identity,process_instance_id}'
                IS DISTINCT FROM deployment_row.snapshot
                    #>> '{panel_certificate,process_instance_id}'
            OR intent_json #>> '{panel,process_identity,process_instance_id}'
                IS DISTINCT FROM deployment_row.snapshot
                    #>> '{panel_certificate,process_instance_id}'
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_intent_invalid';
        END IF;

        expected_intent_bytes :=
            starring_runtime_private_v2.starring_runtime_certification_intent_bytes_v2(
                (intent_json ->> 'action_id')::BIGINT,
                reservation_row.operation_id,
                deployment_row.tenant_id,
                deployment_row.installation_id,
                deployment_row.deployment_id,
                deployment_row.revision,
                deployment_row.controller_id,
                deployment_row.controller_fencing_token,
                deployment_row.runtime_generation,
                deployment_row.convergence_attempt_no,
                deployment_row.guild_id,
                deployment_row.ruleset_key,
                deployment_row.target_version,
                deployment_row.target_content_hash,
                deployment_row.binding_revision,
                deployment_row.binding_fingerprint,
                deployment_row.installation_authority_revision,
                deployment_row.snapshot
                    #>> '{panel_certificate,process_instance_id}',
                'shard:0',
                (
                    intent_json
                        #>> '{gateway_owner_lease_id,lease_epoch}'
                )::BIGINT,
                (intent_json ->> 'observed_owner_revision')::BIGINT,
                intent_json ->> 'runtime_build_revision',
                deployment_row.snapshot
                    #>> '{panel_certificate,certificate_id}',
                deployment_row.snapshot
                    #>> '{panel_certificate,report_digest}',
                (
                    intent_json ->> 'serving_lease_milliseconds'
                )::BIGINT
            );
    EXCEPTION
        WHEN OTHERS THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RX004',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_intent_invalid';
    END;
    IF reservation_row.certification_intent_bytes
            IS DISTINCT FROM expected_intent_bytes
        OR reservation_row.intent_fingerprint IS DISTINCT FROM
            starring_runtime_private_v2.starring_runtime_certification_intent_fingerprint_v2(
                reservation_row.certification_intent_bytes
            )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_intent_invalid';
    END IF;

    authority_outcome := public.starring_runtime_lock_current_authority(
        deployment_row.activation_request_id,
        deployment_row.promotion_id,
        deployment_row.tenant_id,
        deployment_row.installation_id,
        deployment_row.installation_authority_revision,
        deployment_row.guild_id,
        deployment_row.ruleset_key,
        deployment_row.target_version,
        deployment_row.target_content_hash,
        deployment_row.binding_revision,
        deployment_row.binding_fingerprint
    );
    IF authority_outcome = 'active_mismatch' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX006',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_target_superseded';
    ELSIF authority_outcome IS DISTINCT FROM 'exact' THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX003',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_authority_changed';
    END IF;

    mutation_clock := public.starring_runtime_mutation_clock();
    IF mutation_clock < database_now
        OR mutation_clock < requested_minimum_database_now
        OR mutation_clock >= expected_owner_expires_at
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_owner_lost';
    END IF;

    successor_slot_writer_epoch :=
        starring_runtime_private_v2.starring_runtime_slot_writer_fence_begin_unsafe_v2(
            deployment_row.guild_id,
            deployment_row.ruleset_key,
            slot_writer_epoch
        );
    IF successor_slot_writer_epoch <> slot_writer_epoch + 1 THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_slot_invalid';
    END IF;

    SELECT fence.*
    INTO terminal_slot_fence_row
    FROM public.runtime_slot_writer_fences_v2 AS fence
    WHERE fence.slot_guild_id = deployment_row.guild_id
        AND fence.slot_ruleset_key = deployment_row.ruleset_key
    FOR UPDATE;
    IF NOT FOUND
        OR terminal_slot_fence_row.writer_epoch
            IS DISTINCT FROM successor_slot_writer_epoch
        OR terminal_slot_fence_row.pending_drain_intent_id IS NOT NULL
        OR terminal_slot_fence_row.pending_product_operation_id IS NOT NULL
        OR terminal_slot_fence_row.pending_tenant_id IS NOT NULL
        OR terminal_slot_fence_row.pending_installation_id IS NOT NULL
        OR terminal_slot_fence_row.pending_deployment_id IS NOT NULL
        OR terminal_slot_fence_row.pending_expected_revision IS NOT NULL
        OR terminal_slot_fence_row.pending_marked_at IS NOT NULL
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_slot_invalid';
    END IF;

    next_revision := deployment_row.revision + 1;
    next_snapshot := pg_catalog.jsonb_set(
        deployment_row.snapshot,
        '{revision}',
        pg_catalog.to_jsonb(next_revision),
        FALSE
    );
    next_snapshot := pg_catalog.jsonb_set(
        next_snapshot,
        '{phase}',
        '{"phase":"reconciling_panels"}'::JSONB,
        FALSE
    );
    next_snapshot := pg_catalog.jsonb_set(
        next_snapshot,
        '{controller_lease}',
        'null'::JSONB,
        FALSE
    );
    next_snapshot := pg_catalog.jsonb_set(
        next_snapshot,
        '{panel_certificate}',
        'null'::JSONB,
        FALSE
    );
    next_snapshot := pg_catalog.jsonb_set(
        next_snapshot,
        '{gateway_ready}',
        'null'::JSONB,
        FALSE
    );
    next_snapshot := pg_catalog.jsonb_set(
        next_snapshot,
        '{live}',
        'null'::JSONB,
        FALSE
    );
    proposed_deployment := pg_catalog.to_jsonb(deployment_row)
        || pg_catalog.jsonb_build_object(
            'snapshot', next_snapshot,
            'revision', next_revision,
            'phase', 'reconciling_panels',
            'controller_id', NULL::TEXT,
            'controller_fencing_token', NULL::BIGINT,
            'controller_acquired_at', NULL::TIMESTAMPTZ,
            'controller_lease_expires_at', NULL::TIMESTAMPTZ,
            'updated_at', GREATEST(
                mutation_clock,
                deployment_row.updated_at + INTERVAL '1 microsecond'
            )
        );

    source_deployment_frame :=
        pg_catalog.jsonb_send(pg_catalog.to_jsonb(deployment_row));
    successor_deployment_frame :=
        pg_catalog.jsonb_send(proposed_deployment);
    source_slot_frame :=
        pg_catalog.jsonb_send(pg_catalog.to_jsonb(slot_fence_row));
    successor_slot_frame :=
        pg_catalog.jsonb_send(
            pg_catalog.to_jsonb(terminal_slot_fence_row)
        );

    field_bytes := pg_catalog.convert_to(
        reservation_row.operation_id,
        'UTF8'
    );
    reservation_frame :=
        pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.tenant_id,
        'UTF8'
    );
    reservation_frame := reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.installation_id,
        'UTF8'
    );
    reservation_frame := reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.deployment_id,
        'UTF8'
    );
    reservation_frame := reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            reservation_row.deployment_revision
        )
        || pg_catalog.int8send(
            reservation_row.convergence_attempt_no
        )
        || pg_catalog.int8send(
            pg_catalog.octet_length(
                reservation_row.certification_intent_bytes
            )::BIGINT
        )
        || reservation_row.certification_intent_bytes;
    field_bytes := pg_catalog.convert_to(
        reservation_row.intent_fingerprint,
        'UTF8'
    );
    reservation_frame := reservation_frame
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;

    domain_bytes := pg_catalog.convert_to(
        'starring.runtime.certification.awaiting_reset.receipt.v2',
        'UTF8'
    );
    field_bytes := pg_catalog.convert_to(
        requested_recovery_id,
        'UTF8'
    );
    terminal_receipt_bytes :=
        pg_catalog.int8send(
            pg_catalog.octet_length(domain_bytes)::BIGINT
        )
        || domain_bytes
        || pg_catalog.int2send(2::SMALLINT)
        || pg_catalog.int2send(1::SMALLINT)
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            requested_originating_emergency_generation
        )
        || pg_catalog.int8send(requested_coordinator_generation)
        || pg_catalog.int8send(
            requested_action_authority_revision
        )
        || pg_catalog.int8send(
            requested_selection_authority_revision
        )
        || pg_catalog.sha256(source_deployment_frame)
        || pg_catalog.sha256(successor_deployment_frame)
        || pg_catalog.sha256(source_slot_frame)
        || pg_catalog.sha256(successor_slot_frame)
        || pg_catalog.sha256(reservation_frame)
        || pg_catalog.timestamptz_send(mutation_clock);
    IF pg_catalog.octet_length(terminal_receipt_bytes)
            NOT BETWEEN 1 AND 1048576
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_receipt_invalid';
    END IF;

    certification_terminal_digest :=
        starring_runtime_private_v2.starring_runtime_certification_terminal_digest_v2(
            2::SMALLINT,
            reservation_row.operation_id,
            reservation_row.intent_fingerprint,
            reservation_row.tenant_id,
            reservation_row.installation_id,
            reservation_row.deployment_id,
            reservation_row.deployment_revision,
            reservation_row.convergence_attempt_no,
            'awaiting_reset',
            'reconciling_panels',
            next_revision,
            reservation_row.convergence_attempt_no,
            mutation_clock,
            terminal_receipt_bytes
        );

    BEGIN
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_action_v2',
            'insert',
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_operation_id_v2',
            reservation_row.operation_id,
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_outcome_v2',
            'awaiting_reset',
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_result_phase_v2',
            'reconciling_panels',
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_result_revision_v2',
            next_revision::TEXT,
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_result_attempt_v2',
            reservation_row.convergence_attempt_no::TEXT,
            TRUE
        );
        PERFORM pg_catalog.set_config(
            'starring.runtime_certification_terminal_digest_v2',
            certification_terminal_digest,
            TRUE
        );

        INSERT INTO public.runtime_certification_operation_terminals_v2 (
            record_format_version,
            operation_id,
            intent_fingerprint,
            tenant_id,
            installation_id,
            deployment_id,
            deployment_revision,
            convergence_attempt_no,
            terminal_outcome_name,
            resulting_phase,
            resulting_deployment_revision,
            resulting_convergence_attempt_no,
            terminal_at,
            terminal_receipt_bytes,
            terminal_receipt_digest
        ) VALUES (
            2,
            reservation_row.operation_id,
            reservation_row.intent_fingerprint,
            reservation_row.tenant_id,
            reservation_row.installation_id,
            reservation_row.deployment_id,
            reservation_row.deployment_revision,
            reservation_row.convergence_attempt_no,
            'awaiting_reset',
            'reconciling_panels',
            next_revision,
            reservation_row.convergence_attempt_no,
            mutation_clock,
            terminal_receipt_bytes,
            certification_terminal_digest
        )
        RETURNING * INTO terminal_row;
    EXCEPTION
        WHEN unique_violation THEN
            RAISE EXCEPTION USING
                ERRCODE = '40001',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_selection_changed';
    END;

    UPDATE public.runtime_deployments AS deployment
    SET snapshot = next_snapshot,
        revision = next_revision,
        phase = 'reconciling_panels',
        controller_id = NULL,
        controller_fencing_token = NULL,
        controller_acquired_at = NULL,
        controller_lease_expires_at = NULL,
        updated_at = GREATEST(
            mutation_clock,
            deployment.updated_at + INTERVAL '1 microsecond'
        )
    WHERE deployment.tenant_id = deployment_row.tenant_id
        AND deployment.installation_id =
            deployment_row.installation_id
        AND deployment.deployment_id = deployment_row.deployment_id
        AND deployment.revision = deployment_row.revision
    RETURNING deployment.* INTO terminal_deployment_row;
    IF NOT FOUND
        OR pg_catalog.to_jsonb(terminal_deployment_row)
            IS DISTINCT FROM proposed_deployment
        OR NOT starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(
            deployment_row,
            terminal_deployment_row,
            mutation_clock
        )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_transition_invalid';
    END IF;

    terminal_projection_bytes := progressed_projection_prefix
        || pg_catalog.convert_to(
            reservation_row.operation_id,
            'UTF8'
        )
        || pg_catalog.convert_to(
            certification_terminal_digest,
            'UTF8'
        );
    FOREACH field_bytes IN ARRAY ARRAY[
        source_deployment_frame,
        successor_deployment_frame,
        source_slot_frame,
        successor_slot_frame,
        reservation_frame
    ]
    LOOP
        terminal_projection_bytes := terminal_projection_bytes
            || pg_catalog.int8send(
                pg_catalog.octet_length(field_bytes)::BIGINT
            )
            || field_bytes;
    END LOOP;
    terminal_projection_bytes := terminal_projection_bytes
        || pg_catalog.int2send(2::SMALLINT);
    field_bytes := pg_catalog.convert_to(
        terminal_row.terminal_outcome_name,
        'UTF8'
    );
    terminal_projection_bytes := terminal_projection_bytes
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    field_bytes := pg_catalog.convert_to(
        terminal_row.resulting_phase,
        'UTF8'
    );
    terminal_projection_bytes := terminal_projection_bytes
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes
        || pg_catalog.int8send(
            terminal_row.resulting_deployment_revision
        )
        || pg_catalog.int8send(
            terminal_row.resulting_convergence_attempt_no
        )
        || pg_catalog.timestamptz_send(terminal_row.terminal_at)
        || pg_catalog.int8send(
            pg_catalog.octet_length(
                terminal_row.terminal_receipt_bytes
            )::BIGINT
        )
        || terminal_row.terminal_receipt_bytes;
    field_bytes := pg_catalog.convert_to(
        terminal_row.terminal_receipt_digest,
        'UTF8'
    );
    terminal_projection_bytes := terminal_projection_bytes
        || pg_catalog.int8send(
            pg_catalog.octet_length(field_bytes)::BIGINT
        )
        || field_bytes;
    IF pg_catalog.octet_length(terminal_projection_bytes)
            NOT BETWEEN 1 AND 1048576
        OR NOT starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(
            terminal_projection_bytes,
            requested_recovery_id,
            requested_originating_emergency_generation,
            requested_coordinator_generation,
            requested_action_authority_revision,
            requested_selection_authority_revision,
            mutation_clock,
            terminal_row
        ) IS TRUE
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_projection_invalid';
    END IF;

    SELECT record.*
    INTO STRICT action_record
    FROM starring_runtime_private_v2.starring_runtime_startup_recovery_action_record_v2(
        requested_recovery_id,
        requested_originating_emergency_generation,
        requested_coordinator_generation,
        requested_action_authority_revision,
        requested_selection_authority_revision,
        'reserved_awaiting_certification',
        expected_gateway_shard_id,
        expected_owner_process_instance_id,
        expected_owner_lease_epoch,
        expected_owner_runtime_build_revision,
        expected_owner_revision,
        expected_owner_expires_at,
        requested_minimum_database_now,
        terminal_projection_bytes
    ) AS record;
    IF action_record.outcome_name IS DISTINCT FROM 'applied'
        OR action_record.database_now < mutation_clock
        OR action_record.recorded_at < mutation_clock
        OR action_record.database_now
            IS DISTINCT FROM action_record.recorded_at
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RX004',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_record_invalid';
    END IF;

    journal_outcome_name := action_record.outcome_name;
    terminal_outcome_name := 'progressed';
    recovery_id := requested_recovery_id;
    originating_emergency_generation :=
        requested_originating_emergency_generation;
    coordinator_generation := requested_coordinator_generation;
    action_authority_revision :=
        requested_action_authority_revision;
    selection_authority_revision :=
        requested_selection_authority_revision;
    recovery_class := 'reserved_awaiting_certification';
    observed_gateway_shard_id :=
        action_record.observed_gateway_shard_id;
    observed_process_instance_id :=
        action_record.observed_process_instance_id;
    observed_lease_epoch := action_record.observed_lease_epoch;
    observed_runtime_build_revision :=
        action_record.observed_runtime_build_revision;
    observed_owner_revision := action_record.observed_owner_revision;
    database_now := action_record.database_now;
    observed_owner_expires_at :=
        action_record.observed_owner_expires_at;
    minimum_database_now := requested_minimum_database_now;
    recorded_at := action_record.recorded_at;
    terminal_digest := action_record.terminal_digest;
    RETURN NEXT;
END;
$function$;

DO $execution_acl$
DECLARE
    common_owner OID;
    executor_role OID;
    executor_name TEXT;
BEGIN
    SELECT relation.relowner
    INTO common_owner
    FROM pg_catalog.pg_class AS relation
    WHERE relation.oid = pg_catalog.to_regclass(
        'public.runtime_deployments'
    );

    SELECT privilege.grantee
    INTO executor_role
    FROM pg_catalog.pg_proc AS capability
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        capability.proacl,
        pg_catalog.acldefault('f', capability.proowner)
    )) AS privilege
    WHERE capability.oid = pg_catalog.to_regprocedure(
            'public.starring_runtime_execution_database_identity_v1()'
        )
        AND privilege.grantee <> common_owner
    ORDER BY privilege.grantee
    LIMIT 1;


    REVOKE ALL ON FUNCTION
        public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(
            TEXT,
            BIGINT,
            BIGINT,
            BIGINT,
            BIGINT,
            TEXT,
            TEXT,
            BIGINT,
            TEXT,
            BIGINT,
            TIMESTAMPTZ,
            TIMESTAMPTZ
        )
    FROM PUBLIC;

    IF common_owner IS NULL
        OR pg_catalog.to_regrole(current_user) <> common_owner
        OR (
            executor_role IS NOT NULL
            AND executor_role = common_owner
        )
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_acl_drift';
    END IF;

    IF executor_role IS NOT NULL THEN
        executor_name := pg_catalog.pg_get_userbyid(executor_role);
        IF executor_name IS NULL THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RE001',
                MESSAGE = 'runtime_startup_reserved_awaiting_execution_acl_drift';
        END IF;
        EXECUTE pg_catalog.format(
            'GRANT EXECUTE ON FUNCTION public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(TEXT,BIGINT,BIGINT,BIGINT,BIGINT,TEXT,TEXT,BIGINT,TEXT,BIGINT,TIMESTAMPTZ,TIMESTAMPTZ) TO %I',
            executor_name
        );
    END IF;

END;
$execution_acl$;

DO $patch_cross_runtime_manifests$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_exact_target_schema_manifest_v1()'
    );

    previous_fragment :=
        'RETURN observed_count = 356' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''ca4d76873d9256406baaad080943a78b7a6eeeae409ad67e8dc896f0a237642a'';';
    next_fragment :=
        'RETURN observed_count = 356' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''0cf3922bf0d781286003b91b77410aae1d4e02d210534b5a221032fb151346da'';';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_exact_manifest_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;

    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_serving_schema_manifest_v1()'
    );

    previous_fragment :=
        'RETURN observed_count = 471' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''1b476578005a17dadfa9a6f3d26f966e929af5909cdc5097eb6a63050ec310fa'';';
    next_fragment :=
        'RETURN observed_count = 471' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''eb21a116efed629be672561b820c9f525594cdd5a7502f7c60cae00b5f9e051e'';';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_serving_manifest_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_cross_runtime_manifests$;

DO $patch_cross_runtime_readiness$
DECLARE
    definition TEXT;
    expected RECORD;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    FOR expected IN
        SELECT readiness.*
        FROM (
            VALUES
                (
                    'public.starring_runtime_exact_target_database_readiness_v1()',
                    '5fe0365d0cb4912a01778f3d30a2d649a40e82c5b964ba9e2e7e1901e79eb109'::TEXT,
                    '4633e8a3b8dc31d8ddde8d872969b42bdd25a6d98edaf7f59ec3076f3fa4f728'::TEXT
                ),
                (
                    'public.starring_runtime_serving_database_readiness_v1()',
                    '14a0c119d8fa0b7a85b72509df29156a6c869b5e3f240bc8fffc89fd1a86c4c9'::TEXT,
                    '2c8957777b2d4a7f1b6050b21e8a5664b5fcff45d4732627bc8e961823a4eada'::TEXT
                )
        ) AS readiness(identity, previous_digest, next_digest)
    LOOP
        SELECT pg_catalog.pg_get_functiondef(function_row.oid)
        INTO definition
        FROM pg_catalog.pg_proc AS function_row
        WHERE function_row.oid = pg_catalog.to_regprocedure(
            expected.identity
        );

        previous_fragment :=
            pg_catalog.quote_literal(expected.previous_digest) || '::TEXT';
        next_fragment :=
            pg_catalog.quote_literal(expected.next_digest) || '::TEXT';
        IF definition IS NULL
            OR pg_catalog.strpos(definition, previous_fragment) = 0
            OR pg_catalog.strpos(
                pg_catalog.replace(definition, previous_fragment, ''),
                previous_fragment
            ) <> 0
        THEN
            RAISE EXCEPTION USING
                ERRCODE = 'RE001',
                MESSAGE = 'runtime_startup_reserved_awaiting_cross_readiness_patch_drift';
        END IF;
        EXECUTE pg_catalog.replace(
            definition,
            previous_fragment,
            next_fragment
        );
    END LOOP;
END;
$patch_cross_runtime_readiness$;

DO $patch_schema_manifest$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_schema_manifest_v1()'
    );

    previous_fragment :=
        '        SELECT pg_catalog.to_regprocedure(' || E'\n' ||
        '            ''public.starring_runtime_startup_recovery_execute_stale_live_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)''' || E'\n' ||
        '        )';
    next_fragment := previous_fragment || E'\n' ||
        '        UNION' || E'\n' ||
        '        SELECT pg_catalog.to_regprocedure(' || E'\n' ||
        '            ''public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)''' || E'\n' ||
        '        )' || E'\n' ||
        '        UNION' || E'\n' ||
        '        SELECT pg_catalog.to_regprocedure(' || E'\n' ||
        '            ''starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(public.runtime_deployments,public.runtime_deployments,timestamp with time zone)''' || E'\n' ||
        '        )' || E'\n' ||
        '        UNION' || E'\n' ||
        '        SELECT pg_catalog.to_regprocedure(' || E'\n' ||
        '            ''starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(bytea,text,bigint,bigint,bigint,bigint,timestamp with time zone,public.runtime_certification_operation_terminals_v2)''' || E'\n' ||
        '        )';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_manifest_function_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        'RETURN observed_count = 796' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''51f3694196e13c3b5bd21421ccdaa595291f2832063802df4967f502606bf0b5'';';
    next_fragment :=
        'RETURN observed_count = 799' || E'\n' ||
        '        AND observed_digest' || E'\n' ||
        '            = ''52022b152b1189e01928d8cc14dc229d1ed094a5da7837711a06cf3077b0ea41'';';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_manifest_expectation_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_schema_manifest$;

DO $patch_readiness$
DECLARE
    definition TEXT;
    previous_fragment TEXT;
    next_fragment TEXT;
BEGIN
    SELECT pg_catalog.pg_get_functiondef(function_row.oid)
    INTO definition
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_database_readiness_v1()'
    );

    previous_fragment :=
        '            (' || E'\n' ||
        '                ''public.starring_runtime_startup_recovery_execute_stale_live_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)'',' || E'\n' ||
        '                ''requested_recovery_id text, requested_originating_emergency_generation bigint, requested_coordinator_generation bigint, requested_action_authority_revision bigint, requested_selection_authority_revision bigint, expected_gateway_shard_id text, expected_owner_process_instance_id text, expected_owner_lease_epoch bigint, expected_owner_runtime_build_revision text, expected_owner_revision bigint, expected_owner_expires_at timestamp with time zone, requested_minimum_database_now timestamp with time zone''::TEXT,' || E'\n' ||
        '                ''TABLE(journal_outcome_name text, terminal_outcome_name text, recovery_id text, originating_emergency_generation bigint, coordinator_generation bigint, action_authority_revision bigint, selection_authority_revision bigint, recovery_class text, observed_gateway_shard_id text, observed_process_instance_id text, observed_lease_epoch bigint, observed_runtime_build_revision text, observed_owner_revision bigint, database_now timestamp with time zone, observed_owner_expires_at timestamp with time zone, minimum_database_now timestamp with time zone, recorded_at timestamp with time zone, terminal_projection_bytes bytea, terminal_digest text)''::TEXT,' || E'\n' ||
        '                ''plpgsql''::TEXT,' || E'\n' ||
        '                TRUE,' || E'\n' ||
        '                TRUE,' || E'\n' ||
        '                1::REAL' || E'\n' ||
        '            )';
    next_fragment := previous_fragment || ',' || E'\n' ||
        '            (' || E'\n' ||
        '                ''public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)'',' || E'\n' ||
        '                ''requested_recovery_id text, requested_originating_emergency_generation bigint, requested_coordinator_generation bigint, requested_action_authority_revision bigint, requested_selection_authority_revision bigint, expected_gateway_shard_id text, expected_owner_process_instance_id text, expected_owner_lease_epoch bigint, expected_owner_runtime_build_revision text, expected_owner_revision bigint, expected_owner_expires_at timestamp with time zone, requested_minimum_database_now timestamp with time zone''::TEXT,' || E'\n' ||
        '                ''TABLE(journal_outcome_name text, terminal_outcome_name text, recovery_id text, originating_emergency_generation bigint, coordinator_generation bigint, action_authority_revision bigint, selection_authority_revision bigint, recovery_class text, observed_gateway_shard_id text, observed_process_instance_id text, observed_lease_epoch bigint, observed_runtime_build_revision text, observed_owner_revision bigint, database_now timestamp with time zone, observed_owner_expires_at timestamp with time zone, minimum_database_now timestamp with time zone, recorded_at timestamp with time zone, terminal_projection_bytes bytea, terminal_digest text)''::TEXT,' || E'\n' ||
        '                ''plpgsql''::TEXT,' || E'\n' ||
        '                TRUE,' || E'\n' ||
        '                TRUE,' || E'\n' ||
        '                1::REAL' || E'\n' ||
        '            )';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_readiness_function_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '            (''starring_runtime_private_v2.starring_runtime_certification_terminal_digest_v2(smallint,text,text,text,text,text,bigint,bigint,text,text,bigint,bigint,timestamp with time zone,bytea)''),';
    next_fragment := previous_fragment || E'\n' ||
        '            (''starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(public.runtime_deployments,public.runtime_deployments,timestamp with time zone)''),' || E'\n' ||
        '            (''starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(bytea,text,bigint,bigint,bigint,bigint,timestamp with time zone,public.runtime_certification_operation_terminals_v2)''),';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_readiness_private_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '''1fa238c260d3bdfa7b0c914a42616c2889c02d92253829c8bafa63a8a255a3f7''::TEXT';
    next_fragment :=
        '''c2de6cf64ce6efbcf22e31f06da774195996060a692c45b48f073ff93fa4d630''::TEXT';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_readiness_manifest_digest_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );

    previous_fragment :=
        '            pg_catalog.to_regprocedure(' || E'\n' ||
        '                ''public.starring_runtime_startup_recovery_execute_stale_live_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)''' || E'\n' ||
        '            )' || E'\n' ||
        '        )';
    next_fragment :=
        '            pg_catalog.to_regprocedure(' || E'\n' ||
        '                ''public.starring_runtime_startup_recovery_execute_stale_live_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)''' || E'\n' ||
        '            ),' || E'\n' ||
        '            pg_catalog.to_regprocedure(' || E'\n' ||
        '                ''public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)''' || E'\n' ||
        '            )' || E'\n' ||
        '        )';
    IF pg_catalog.strpos(definition, previous_fragment) = 0
        OR pg_catalog.strpos(
            pg_catalog.replace(definition, previous_fragment, ''),
            previous_fragment
        ) <> 0
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_readiness_allowlist_patch_drift';
    END IF;
    definition := pg_catalog.replace(
        definition,
        previous_fragment,
        next_fragment
    );
    EXECUTE definition;
END;
$patch_readiness$;

DO $postflight$
DECLARE
    common_owner OID;
    executor_role OID;
    invalid_function_count BIGINT;
    invalid_acl_count BIGINT;
    actual_acl_count BIGINT;
    expected_acl_count BIGINT;
    executor_identity_acl_count BIGINT;
    invalid_executor_identity_acl_count BIGINT;
    exact_target_manifest_digest TEXT;
    exact_target_readiness_digest TEXT;
    serving_manifest_digest TEXT;
    serving_readiness_digest TEXT;
    manifest_digest TEXT;
    readiness_digest TEXT;
    observation_digest TEXT;
    executor_digest TEXT;
    deployment_validator_digest TEXT;
    convergence_validator_digest TEXT;
BEGIN
    SELECT relation.relowner
    INTO common_owner
    FROM pg_catalog.pg_class AS relation
    WHERE relation.oid = pg_catalog.to_regclass(
        'public.runtime_deployments'
    );

    SELECT privilege.grantee
    INTO executor_role
    FROM pg_catalog.pg_proc AS capability
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        capability.proacl,
        pg_catalog.acldefault('f', capability.proowner)
    )) AS privilege
    WHERE capability.oid = pg_catalog.to_regprocedure(
            'public.starring_runtime_execution_database_identity_v1()'
        )
        AND privilege.grantee <> common_owner
    ORDER BY privilege.grantee
    LIMIT 1;

    SELECT
        pg_catalog.count(*) FILTER (
            WHERE privilege.grantee <> common_owner
        ),
        pg_catalog.count(*) FILTER (
            WHERE privilege.grantee <> common_owner
                AND (
                    privilege.grantor <> common_owner
                    OR privilege.privilege_type <> 'EXECUTE'
                    OR privilege.is_grantable
                    OR privilege.grantee IS DISTINCT FROM executor_role
                )
        )
    INTO
        executor_identity_acl_count,
        invalid_executor_identity_acl_count
    FROM pg_catalog.pg_proc AS capability
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        capability.proacl,
        pg_catalog.acldefault('f', capability.proowner)
    )) AS privilege
    WHERE capability.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_database_identity_v1()'
    );

    SELECT pg_catalog.count(*)
    INTO invalid_function_count
    FROM (
        VALUES
            (
                'public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)',
                'v'::"char",
                'u'::"char",
                TRUE,
                TRUE
            ),
            (
                'starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(public.runtime_deployments,public.runtime_deployments,timestamp with time zone)',
                's'::"char",
                'u'::"char",
                FALSE,
                FALSE
            ),
            (
                'starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(bytea,text,bigint,bigint,bigint,bigint,timestamp with time zone,public.runtime_certification_operation_terminals_v2)',
                's'::"char",
                'u'::"char",
                FALSE,
                FALSE
            )
    ) AS expected(
        identity,
        volatility,
        parallel_kind,
        security_definer,
        returns_set
    )
    LEFT JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid =
            pg_catalog.to_regprocedure(expected.identity)
    WHERE function_row.oid IS NULL
        OR function_row.proowner <> common_owner
        OR function_row.prokind <> 'f'
        OR function_row.provolatile <> expected.volatility
        OR NOT function_row.proisstrict
        OR function_row.proparallel <> expected.parallel_kind
        OR function_row.prosecdef <> expected.security_definer
        OR function_row.proretset <> expected.returns_set
        OR function_row.proleakproof
        OR function_row.pronargdefaults <> 0
        OR function_row.provariadic <> 0
        OR function_row.proconfig
            IS DISTINCT FROM ARRAY['search_path=pg_catalog']::TEXT[];

    SELECT pg_catalog.count(*)
    INTO invalid_acl_count
    FROM (
        VALUES
            (
                'public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)',
                TRUE
            ),
            (
                'starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(public.runtime_deployments,public.runtime_deployments,timestamp with time zone)',
                FALSE
            ),
            (
                'starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(bytea,text,bigint,bigint,bigint,bigint,timestamp with time zone,public.runtime_certification_operation_terminals_v2)',
                FALSE
            )
    ) AS expected(identity, executor_allowed)
    INNER JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid =
            pg_catalog.to_regprocedure(expected.identity)
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        function_row.proacl,
        pg_catalog.acldefault('f', function_row.proowner)
    )) AS privilege
    WHERE privilege.grantor <> common_owner
        OR privilege.privilege_type <> 'EXECUTE'
        OR privilege.is_grantable
        OR (
            privilege.grantee <> common_owner
            AND NOT (
                expected.executor_allowed
                AND executor_role IS NOT NULL
                AND privilege.grantee = executor_role
            )
        );

    SELECT pg_catalog.count(*)
    INTO actual_acl_count
    FROM (
        VALUES
            (
                'public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)'
            ),
            (
                'starring_runtime_private_v2.starring_runtime_cert_awaiting_reset_exact_v2(public.runtime_deployments,public.runtime_deployments,timestamp with time zone)'
            ),
            (
                'starring_runtime_private_v2.starring_runtime_startup_reserved_projection_exact_v2(bytea,text,bigint,bigint,bigint,bigint,timestamp with time zone,public.runtime_certification_operation_terminals_v2)'
            )
    ) AS expected(identity)
    INNER JOIN pg_catalog.pg_proc AS function_row
        ON function_row.oid =
            pg_catalog.to_regprocedure(expected.identity)
    CROSS JOIN LATERAL pg_catalog.aclexplode(COALESCE(
        function_row.proacl,
        pg_catalog.acldefault('f', function_row.proowner)
    )) AS privilege;
    expected_acl_count := 3 + CASE
        WHEN executor_role IS NULL THEN 0
        ELSE 1
    END;

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO exact_target_manifest_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_exact_target_schema_manifest_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO exact_target_readiness_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_exact_target_database_readiness_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO serving_manifest_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_serving_schema_manifest_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO serving_readiness_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_serving_database_readiness_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO manifest_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_schema_manifest_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO readiness_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_execution_database_readiness_v1()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO observation_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_startup_recovery_observe_v2(text,text,bigint,text,bigint,timestamp with time zone)'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO executor_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.starring_runtime_startup_recovery_execute_reserved_awaiting_v2(text,bigint,bigint,bigint,bigint,text,text,bigint,text,bigint,timestamp with time zone,timestamp with time zone)'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO deployment_validator_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.validate_runtime_deployment_projection()'
    );

    SELECT pg_catalog.encode(
        pg_catalog.sha256(pg_catalog.convert_to(
            pg_catalog.pg_get_functiondef(function_row.oid),
            'UTF8'
        )),
        'hex'
    )
    INTO convergence_validator_digest
    FROM pg_catalog.pg_proc AS function_row
    WHERE function_row.oid = pg_catalog.to_regprocedure(
        'public.validate_runtime_convergence_attempt_projection()'
    );

    IF common_owner IS NULL
        OR pg_catalog.to_regrole(current_user) <> common_owner
        OR invalid_function_count <> 0
        OR invalid_acl_count <> 0
        OR actual_acl_count <> expected_acl_count
        OR executor_identity_acl_count <> (
            CASE
                WHEN executor_role IS NULL THEN 0
                ELSE 1
            END
        )
        OR invalid_executor_identity_acl_count <> 0
        OR manifest_digest IS DISTINCT FROM
            'c2de6cf64ce6efbcf22e31f06da774195996060a692c45b48f073ff93fa4d630'
        OR readiness_digest IS DISTINCT FROM
            '4e58c914016de080372586cc2efc7e9a5221c8703450d767934389a5c4c07db8'
        OR observation_digest IS DISTINCT FROM
            'bd2844074b41d4c8723c44ce482be8aa943b2354904de42bf9260dce0376aab1'
        OR executor_digest IS DISTINCT FROM
            '58de1296bfae9c48565a656decc86b6fa9f6a84a1caf5d8bf7dafb0c2bdc0a34'
        OR deployment_validator_digest IS DISTINCT FROM
            '6b9d750c7ddee0d4e4c2034efaaaedfa8e45dca96f1e9d709360731cabd012c4'
        OR convergence_validator_digest IS DISTINCT FROM
            'a8be65eb734c5b1fa50edadecbe4547ea96f594800b984d36d327cff246ff252'
        OR exact_target_manifest_digest IS DISTINCT FROM
            '4633e8a3b8dc31d8ddde8d872969b42bdd25a6d98edaf7f59ec3076f3fa4f728'
        OR exact_target_readiness_digest IS DISTINCT FROM
            '5110e2d4b5846a64550e2ed55c219a0d010d3ed944a986eca2e927ce345276ad'
        OR serving_manifest_digest IS DISTINCT FROM
            '2c8957777b2d4a7f1b6050b21e8a5664b5fcff45d4732627bc8e961823a4eada'
        OR serving_readiness_digest IS DISTINCT FROM
            'aa6b4f686ff76b627a7850e6ea110cb5ecf4c2e20c1a252b56b9b91bfee1cf27'
        OR NOT public.starring_runtime_exact_target_schema_manifest_v1()
        OR NOT public.starring_runtime_serving_schema_manifest_v1()
        OR NOT public.starring_runtime_execution_schema_manifest_v1()
    THEN
        RAISE EXCEPTION USING
            ERRCODE = 'RE001',
            MESSAGE = 'runtime_startup_reserved_awaiting_execution_postflight_drift';
    END IF;
END;
$postflight$;

RESET statement_timeout;
RESET lock_timeout;
RESET search_path;
