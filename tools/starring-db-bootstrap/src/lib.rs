...
pub const CAPABILITY_FUNCTION_COUNT: usize = 95;
...
mod tests {
    ...
    fn manifest_is_exact_and_unique() {
        ...
        assert_eq!(identities.len(), 95);
        assert_eq!(
            identities.iter().copied().collect::<BTreeSet<_>>().len(),
            95
        );
        assert_eq!(extract_manifest(API_ROLE_BOOTSTRAP).len(), 46);
    ...
    fn fixed_identities_are_not_configurable() {
        ...
        assert_eq!(CAPABILITY_FUNCTION_COUNT, 95);
