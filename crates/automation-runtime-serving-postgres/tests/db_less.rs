...
use automation_runtime_serving_postgres::{
    ...
    RuntimeServingDatabaseTimeoutsV1,
};
...
async fn verified_factory_fails_closed_when_database_is_unreachable() {
    let pool = PgPoolOptions::new()
        ...
        .connect_lazy("postgresql://starring_runtime_serving:unused@127.0.0.1:1/starring_runtime")
        .unwrap();
    ...
    .unwrap();
    let timeouts = RuntimeServingDatabaseTimeoutsV1::new(
        Duration::from_millis(100),
        Duration::from_millis(50),
    )
    .unwrap();
    let error = PostgresRuntimeServingLeaseV1::connect_verified(pool, expectation, timeouts)
        .await
        .err()
