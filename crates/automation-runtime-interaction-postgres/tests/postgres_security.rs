...
fn ack_first_effect_plan_bind_fix_is_head_exact_and_metadata_preserving() {
    ...
    assert_eq!(versions.get(fix + 3), Some(&202_608_040_001));
    assert_eq!(versions.get(fix + 4), Some(&202_608_040_002));
    assert_eq!(versions.get(fix + 5), Some(&202_608_040_003));
    assert_eq!(versions.get(fix + 6), Some(&202_608_040_004));
    assert_eq!(fix + 6, versions.len() - 1);
...
async fn response_tail_scan_fix_upgrades_from_effect_journal_and_serves_empty_scan() {
    ...
    .await
    .unwrap();
    assert_eq!(ledger, (125, 202_608_040_004, 48));
