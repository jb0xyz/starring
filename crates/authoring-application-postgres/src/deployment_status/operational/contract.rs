...
pub(super) const STATUS_FUNCTION: &str = "public.starring_product_operational_deployment_status_read_v3(text,text,text,text,text,text,text,text,bytea)";
...
pub(super) const STATUS_QUERY: &str = "SELECT request_outcome, deployment_projection, \
    ...
    attestation_convergence_attempt_no, attestation_record_format_version, \
    attestation_serving_lease_duration_nanos, deployment_last_controller_id, \
    v2_evidence_state, v2_operation_id, v2_intent_fingerprint, \
    v2_certification_intent_bytes, v2_request_digest, v2_request_bytes, \
...
mod tests {
    ...
    #[test]
    fn operational_contract_preserves_attempts_and_appends_v2_evidence() {
        assert_eq!(STATUS_ARGUMENTS.matches(',').count() + 1, 9);
        assert_eq!(STATUS_RESULT.matches(',').count() + 1, 29);
        ...
        assert!(STATUS_QUERY
            .contains("FROM public.starring_product_operational_deployment_status_read_v3("));
