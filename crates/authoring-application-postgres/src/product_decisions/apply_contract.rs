...
pub(super) const CONSUME_RUNTIME_DRAIN_FUNCTION: &str = "public.starring_product_apply_consume_runtime_drain_v2(text,text,text,text,bigint,text,text,bytea,bytea,text,text,text,text,bigint,text,text,timestamp with time zone,timestamp with time zone,text,boolean,text,text,text[],text[],text[],text,text,text,text,text,text,text,bigint,bytea,text,text,text,bigint,text,text,bytea,text,bytea,text,text,bytea)";
pub(super) const CONSUME_RUNTIME_DRAIN_ARGUMENTS: &str = "requested_phase text, expected_tenant_id text, expected_installation_id text, expected_promotion_id text, expected_product_revision bigint, expected_payload_digest text, expected_principal_id text, expected_product_session_digest bytea, session_subject_digest bytea, expected_acting_user_id text, expected_discord_application_id text, expected_guild_id text, expected_capability text, expected_authority_revision bigint, expected_authority_payload_digest text, expected_authority_observation_digest text, expected_authority_observed_at timestamp with time zone, expected_authority_expires_at timestamp with time zone, expected_effective_permission_bits text, expected_guild_owner boolean, product_request_id text, active_idempotency_key_digest text, idempotency_key_digest_candidates text[], idempotency_digest_key_id_candidates text[], idempotency_digest_key_fingerprint_candidates text[], idempotency_digest_key_id text, semantic_request_digest text, new_receipt_id text, new_audit_event_id text, new_apply_attempt_id text, new_deployment_id text, expected_drain_intent_id text, expected_source_intent_revision bigint, expected_source_state_bytes bytea, expected_source_state_digest text, expected_product_operation_id text, expected_source_deployment_id text, expected_source_deployment_revision bigint, proposed_terminal_action_id text, expected_preparation_token text, prepared_source_result_snapshot_bytes bytea, prepared_source_result_snapshot_digest text, prepared_result_deployment_snapshot_bytes bytea, prepared_result_deployment_snapshot_digest text, prepared_desired_target_digest text, prepared_activation_notices_bytes bytea";
pub(super) const CONSUME_RUNTIME_DRAIN_RESULT: &str = "TABLE(outcome_name text, preparation_ready boolean, exact_replay boolean, requires_commit boolean, preparation_token text, locked_product_projection jsonb, source_deployment_snapshot jsonb, source_acknowledged_at timestamp with time zone, product_operation_id text, product_mutation_digest text, drain_intent_digest text, source_deployment_id text, source_deployment_revision bigint, source_result_deployment_revision bigint, source_result_deployment_snapshot jsonb, source_result_deployment_snapshot_digest text, result_deployment_id text, result_deployment_revision bigint, result_deployment_snapshot jsonb, result_deployment_snapshot_digest text, product_resulting_revision bigint, product_resulting_state text, product_receipt_id text, product_audit_event_id text, drain_intent_id text, source_intent_revision bigint, source_state_bytes bytea, source_state_digest text, result_intent_revision bigint, result_intent_state text, result_state_bytes bytea, result_state_digest text, source_slot_epoch bigint, successor_slot_epoch bigint, terminal_action_id text, terminal_projection_bytes bytea, terminal_projection_digest text, terminal_database_time timestamp with time zone)";
