...
WITH common_owner AS (
    ...
    VALUES
        ...
        ('public.product_action_receipts', 'product_action_receipts_approval_key_identity_required', 'c'::"char", FALSE,
            'CHECK (((endpoint_domain <> ALL (ARRAY[''product_approve_v1''::text, ''product_apply_v1''::text, ''product_promote_v1''::text])) OR ((idempotency_digest_key_id IS NOT NULL) AND (idempotency_digest_key_fingerprint IS NOT NULL))))'),
