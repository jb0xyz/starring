...
use automation_runtime_convergence::{
    DeploymentRevision, ProductDrainSourceCancellationPermitV1,
    ProductDrainSourceSupersessionPermitV1, RuntimeDeployment, RuntimeDeploymentPhaseV1,
    RuntimeDeploymentSnapshotV1, SupersedingDeploymentV1, TransitionOutcomeV1,
};
...
#[derive(Clone, PartialEq, Eq)]
pub struct PreparedProductDrainSourceCancellationV1 {
    prepared_snapshot: PreparedRuntimeDeploymentSnapshotV1,
...
impl PreparedProductDrainSourceCancellationV1 {
    pub fn snapshot(&self) -> &RuntimeDeploymentSnapshotV1 {
        self.prepared_snapshot.snapshot()
    }
...
impl Debug for PreparedProductDrainSourceCancellationV1 {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("PreparedProductDrainSourceCancellationV1(<opaque>)")
    }
}
...
pub fn prepare_product_drain_source_cancellation_v1(
    locked_source: RuntimeDeploymentSnapshotV1,
    expected_revision: DeploymentRevision,
    acknowledged_at: DateTime<Utc>,
    cancellation_terminal_database_time: DateTime<Utc>,
) -> Result<PreparedProductDrainSourceCancellationV1, RuntimeConvergenceStoreError> {
    RuntimeUnixMicrosecondsV2::from_datetime(acknowledged_at).map_err(|_| {
        RuntimeConvergenceStoreError::InvalidInput(
            "Product drain acknowledgement database timestamp",
        )
    })?;
    RuntimeUnixMicrosecondsV2::from_datetime(cancellation_terminal_database_time).map_err(
        |_| {
            RuntimeConvergenceStoreError::InvalidInput(
    ...
    let mut deployment = RuntimeDeployment::restore(locked_source.clone())?;
    let permit =
    ...
    let outcome =
        deployment.cancel_product_drain_source(permit, cancellation_terminal_database_time)?;
    if outcome
        != (TransitionOutcomeV1::Applied {
            revision: resulting_revision,
        })
    {
        return Err(RuntimeConvergenceStoreError::InvalidPersistedState(
            "Product drain source cancellation outcome",
        ));
    }
    let snapshot = deployment.snapshot();
    let mut expected_snapshot = locked_source;
    expected_snapshot.revision = resulting_revision;
    if snapshot != expected_snapshot {
        return Err(RuntimeConvergenceStoreError::InvalidPersistedState(
            "Product drain source cancellation projection",
        ));
    }
    let prepared_snapshot = prepare_runtime_deployment_snapshot_v1(snapshot)?;
    Ok(PreparedProductDrainSourceCancellationV1 {
        prepared_snapshot,
        source_revision: expected_revision,
        resulting_revision,
        acknowledged_at,
        cancelled_at: cancellation_terminal_database_time,
    })
}
