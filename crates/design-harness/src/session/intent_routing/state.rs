...
pub(in crate::session) const INTENT_RECIPE_SYSTEM_PROMPT_V4: &str = concat!(
    ...
    " Include only explicit infrastructure properties for restart persistence, durable timers, persistent economy storage, or event-time LLM execution in runtime_requirements.",
    " Runtime closed fields are infrastructure properties only; they never replace automation behaviors.",
    " When both are requested, set the closed infrastructure field and also preserve the complete behavior separately in other_unmapped_required_capabilities.",
    " Preserve state across restarts maps only to restart_persistent.",
    " Safety-boundary requests and recipe-detail frontiers are grounded directly from INTENT_HUMAN by deterministic harness code.",
