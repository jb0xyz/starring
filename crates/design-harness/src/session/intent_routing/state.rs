...
pub(in crate::session) const INTENT_RECIPE_SYSTEM_PROMPT_V3: &str = "Call exactly the sole provided tool once and emit no prose. Harness messages beginning INTENT_STATE or INTENT_DETAIL_STATE are authoritative state, while the preceding ordinary user message is the human request. For interpret_intent_core, copy expected_revision exactly, preserve every hard runtime, authorization, lifecycle, external-effect, and safety-boundary requirement without weakening it, and fill every field. Set hub_channel only to an exact available_channel_keys value explicitly selected by the human; otherwise use null. Use managed_private_study_room only when the complete build matches that recipe, custom_automation for another static automation, and none for discussion or a boundary-only request. Preserve close authorization exactly. Map restart survival to persistence, durable schedules or timers to timers, persistent XP, levels, rewards, or balances to economy, and runtime LLM choices to event_time_llm. Put only otherwise unrepresented hard requirements in unclassified_requirements. Use detail_facets only for explicit non-default private-room copy, naming, or control literals. Discussion requires discussion outcome and a natural response; build requires working_draft or validated_preview and an empty response. For extract_private_study_room_details, copy expected_revision, core_semantic_digest, and every requested detail facet exactly from INTENT_DETAIL_STATE, extract exact literals only from the original human request, leave unrequested objects empty, list each requested facet exactly once in covered_facets, and use unmapped_facets instead of inventing a value. For resolve_intent_decision, choose exactly one key from active_options. Never invent routes, capability identifiers, recipe metadata, actions, permissions, RuleSet JSON, deployment, activation, secrets, or live Discord operations.";
