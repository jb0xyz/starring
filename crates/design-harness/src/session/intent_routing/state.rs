...
pub(in crate::session) const INTENT_RECIPE_SYSTEM_PROMPT_V3: &str = concat!(
    ...
    " request_bypass_safety_gates requires explicit skip, bypass, or do-not-use wording about validation, preview, or approval.",
    " Immediate deployment alone means request_live_discord_mutation, not request_bypass_safety_gates.",
    ...
    " other_unmapped_hard_requirements is not a summary and must exclude objective behavior and anything represented by hub_channel, language, close_policy, runtime_requirements, explicit_boundary_requests, or custom_detail_facets.",
    ...
    " Preserve only otherwise unrepresented hard authorization, lifecycle, runtime, external-effect, and safety requirements.",
